...
def theme_tags_for(
    ...
    for theme_id, clue_by_answer in THEME_TINTED_CLUES.items():
        if answer in clue_by_answer:
            tags.add(theme_id)
    raw_clues = clue_options if clue_options else (curated["clues"] if curated else ())
    for clue in raw_clues:
        tags.update(clue_theme_tags(answer, clue))
    return sorted(tags)
...
def clue_metadata_for(answer: str, fallback_clues: Sequence[str] = ()) -> List[dict]:
    ...
    for raw, explicit_theme_tags, source in themed_raw_clues:
        ...
        if key in seen:
            continue
        seen.add(key)
        theme_tags = sorted(set(explicit_theme_tags).union(clue_theme_tags(answer, clue)))
        metadata.append(
            {
                "text": clue,
