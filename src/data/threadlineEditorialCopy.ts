...
const DOMAIN_TITLE_EXTENSIONS: Readonly<Record<string, readonly string[]>> = {
  ...
  harbor: ['Water Before Leaving', 'Dock Before Motion', 'Bell Near The Slip', 'The Boat Waits Close', 'Morning At The Mooring', 'The Still Dock', 'Castoff Light', 'Before The Harbor Moves', 'Rope Before Castoff', 'The Boat Still Tied', 'Water Holding Still', 'Before The Dock Lets Go', 'Harbor In The Bell', 'Rope At Morning'],
...
const DOMAIN_AUTHORED_WEAVES: Readonly<Record<string, readonly string[]>> = {
  ...
  harbor: [
    'Still water becomes departure at the dock.',
    'The edge is the moment before distance.',
    'A dock is a held breath before open water.',
    'The dock is where still water starts leaving.',
    ...
    'The dock keeps departure from becoming sudden.',
    'Distance starts as a careful leaving.',
    'The dock turns waiting toward open water.',
...
const THREADLINE_APPROVED_COPY_OVERRIDES: Readonly<Record<string, ThreadlineApprovedCopyOverride>> = {
  ...
  '2026-05-22': {
    domain: 'harbor',
    title: 'Castoff Light',
    leadTemplate: 'At the {A3}, the {A1} and {A2} sit on the dock before {B1}, {B2}, and {B3} begin on the water.',
    weave: 'Leaving starts while the boat is still tied.',
    note: 'Harbor override retires the "name the still edge" and harbor-rail scaffolds while giving marina a believable place role.',
  },
  '2026-05-15': {
    domain: 'bakery',
  ...
  '2026-06-16': {
    domain: 'harbor',
    title: 'Rope In Morning Light',
    leadTemplate: 'At the dock, {A} keep the boat close until {B} pull it toward open water.',
    weave: 'A harbor is a pause with departure inside.',
    note: 'Harbor override replaces water-ahead-means and false rail syntax with a dock-to-open-water sentence.',
  },
  '2026-06-25': {
    domain: 'kitchen',
  ...
  '2026-08-23': {
    domain: 'bakery',
    title: 'Small Errand',
    leadTemplate: 'Behind the glass, {A1}, {A2}, and {A3} hold the line while {B2}, {B1}, and {B3} turn one choice into a paper box.',
    weave: 'The box closes around one small choice.',
    note: 'Bakery override replaces line-tempts machinery with a case-to-box sentence where the gerunds read like actual counter work.',
  },
  '2026-08-27': {
    domain: 'laundry',
    title: 'Folded Proof',
  ...
  '2026-09-09': {
    domain: 'studio',
    leadTemplate: 'On the worktable, {A1} leans near the {A3}, the {A2} waits in the light, and {B1}, {B2}, and {B3} are still possible.',
    weave: 'Materials become marks under a hand.',
    note: 'Studio override replaces draft-visibility phrasing with a hand-before-mark sentence that keeps the materials and marks natural.',
  },
  '2026-09-15': {
    domain: 'school',
  ...
  '2026-10-10': {
    ...
    leadTemplate: 'Before the bell, the {A2} lands beside the {A3}, the {A1} catches the window light, and the teacher is ready with {B1}, {B2}, and {B3}.',
    weave: 'The bell only starts it; the voices make it school.',
    note: 'School override replaces static desk inventory with a child/teacher moment where projects, answers, and welcome no longer need a strained article.',
  },
  '2026-11-01': {
    domain: 'market',
    title: 'One More Stop',
  ...
  '2026-12-31': {
    domain: 'rooftop',
    title: 'Bright Edge',
    leadTemplate: 'From the roof rail, {A1} catches the {A2} and {A3} while {B1}, {B2}, and {B3} settle over the street.',
    weave: 'Up high, the day has room to end.',
    note: 'Rooftop override retires last-light duplication while making the visible skyline and evening texture read as one sentence.',
  },
  '2027-01-03': {
    domain: 'music',
  ...
  '2027-03-18': {
    domain: 'harbor',
    leadTemplate: 'At the dock, a {A1}, {A2}, and a {A3} are fixed in place while {B1}, {B2}, and {B3} begin on the water.',
    weave: 'The dock holds the boat only briefly.',
    note: 'Harbor override retires the still-edge/work-is scaffold and keeps dock fixtures separate from water motion.',
  },
  '2027-04-03': {
    domain: 'school',
  ...
  '2027-04-12': {
    domain: 'harbor',
    leadTemplate: 'Beside the slips, the {A1} holds, the {A2} is closed, and the {A3} waits before {B1}, {B2}, and {B3} begin.',
    weave: 'A tied boat already belongs to open water.',
    note: 'Harbor override replaces boat-turns-to, rail, and repeated still-edge phrasing with a tied-boat/open-water aha.',
  },
  '2027-05-13': {
    domain: 'firehouse',
    title: 'Fast Help',
  ...
  '2027-09-03': {
    domain: 'campsite',
    title: 'Open Air Home',
    leadTemplate: 'After dark, the {A1} hisses near the {A2}, the {A3} stays warm, and someone tends the {B1}, keeps supper {B2}, and keeps {B3} the pot.',
    weave: 'The fire gives the dark a center.',
    note: 'Campsite override replaces utility-slot grammar with a lived fire-ring sequence where each gerund has a believable object.',
  },
  '2027-10-25': {
    domain: 'laboratory',
    title: 'Careful Mark',
  ...
  'threadline-2027-11-10-lighthouse': {
    ...
    leadTemplate: 'Inside the tower, {A1}, {A2}, and {A3} face the dark while the warning can {B1}, {B2}, and {B3} across the water.',
    weave: 'A far light keeps fear from guessing.',
    note: 'Lighthouse ready-reserve override removes the "light can" verb-list scaffold and gives each coast cue a natural object.',
  },
  ...
  'threadline-2027-11-11-weather-station': {
    domain: 'weather-station',
    title: 'A Little Notice',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} keep the air legible while forecasters {B1} it {B2} and {B3}.',
    weave: 'Bad weather is kinder when it comes with notice.',
    note: 'Weather-station ready-reserve override removes catch-a-watch and article-mismatch grammar by making watch, settle, and deepen one observed weather change.',
  },
  'threadline-2027-11-13-weather-station': {
    domain: 'weather-station',
    title: 'Early Warning Desk',
    leadTemplate: 'At the weather desk, the {A1}, {A2}, and {A3} keep the map honest while pressure can {B1}, wind can {B2}, and a gray morning can {B3}.',
    weave: 'A forecast gives change a little warning.',
    note: 'Weather-station ready-reserve override removes staff-drizzle, roads-falling, and article-mismatch grammar by assigning each change to a believable weather subject.',
  },
  ...
  'threadline-2027-11-14-lighthouse': {
    domain: 'lighthouse',
    title: 'Kindness From Above',
    leadTemplate: 'Above the water, {A1}, {A2}, and {A3} keep watch as a {B1} becomes a {B2} and then a {B3}.',
    weave: 'A high light gives fear a bearing.',
    note: 'Lighthouse puzzle-id override avoids the light-can-verb list and preserves the warning-over-water relationship as a visible signal sequence.',
  },
  ...
  'threadline-2026-09-05-desk': {
    domain: 'desk',
    title: 'Clean Start',
    leadTemplate: 'Before work begins, the {A1}, {A2}, and {A3} frame the morning\'s first {B1}, {B2}, and {B3}.',
    weave: 'The day begins when the desk gets specific.',
    note: 'Desk puzzle-id override removes the narrowing-morning formula and lets plan, focus, and launch read like a natural first task.',
  },
  'threadline-2026-06-24-station': {
    domain: 'station',
    title: 'Late Platform',
    leadTemplate: 'On the platform, {A1}, {A2}, and {A3} name the leaving while travelers use {B1}, {B2}, and {B3} to pass the minutes.',
    weave: 'The station turns waiting toward leaving.',
    note: 'Station puzzle-id override removes pass-the-minutes abstraction and gives checkin, headset, and scroll a traveler subject.',
  },
  'threadline-2026-10-09-park': {
    domain: 'park',
    title: 'Park Greeting',
    leadTemplate: 'Around the loop, the {A1}, {A2}, and {A3} stay familiar as people {B1}, {B2}, and {B3}.',
    weave: 'The walk belongs to whoever repeats it.',
    note: 'Park puzzle-id override removes familiar-loop-has syntax and lets the path details sit inside ordinary park behavior.',
  },
  'threadline-2026-10-15-trail': {
    domain: 'trail',
    title: 'Ridge Signal',
    leadTemplate: 'On the first mile, the {A1} reads the {A2} on the {A3} before {B1}, {B2}, and {B3} open around the path.',
    weave: 'Directions begin the walk; wonder keeps it going.',
    note: 'Trail puzzle-id override turns camper, marker, and trail into a readable hiking moment before the natural details arrive.',
  },
  'threadline-2026-11-07-mailroom': {
    domain: 'mailroom',
    title: 'Doorway Route',
    leadTemplate: 'In the mailroom, {B1}, {B2}, and {B3} carry the {A1}, {A2}, and {A3} toward the right door.',
    weave: 'The right door turns sorting into trust.',
    note: 'Mailroom puzzle-id override removes route-gets-built language and gives the delivery-step gerunds a clerk subject.',
  },
  'threadline-2026-11-20-garden': {
    domain: 'garden',
    title: 'Path After Watering',
    leadTemplate: 'After rain, {A1}, {A2}, and {A3} show what needs care while {B1}, {B2}, and {B3} fill the morning.',
    weave: 'A garden is care made visible.',
    note: 'Garden puzzle-id override removes morning-disappears phrasing and gives the tending gerunds a plain after-rain sentence.',
  },
  'threadline-2026-11-13-harbor': {
    domain: 'harbor',
    title: 'Where The Day Starts',
    leadTemplate: 'At the marina, the {A2} checks the {A1} and {A3}, with {B1}, {B2}, and {B3} still ahead.',
    weave: 'Open water starts with a careful check.',
    note: 'Harbor puzzle-id override keeps pilot as a person, removes object-list checking, and connects dock preparation to motion.',
  },
  'threadline-2027-01-02-harbor': {
    domain: 'harbor',
    title: 'Marina Awake',
    leadTemplate: 'At the marina, the {A1}, {A2}, and {A3} sit along the dock, with {B1}, {B2}, and {B3} still ahead.',
    weave: 'Leaving begins before the hull lets go.',
    note: 'Harbor puzzle-id override replaces boat-waits-beside shorthand with a marina sentence that keeps dock details and water motion natural.',
  },
  'threadline-2026-12-03-theater': {
    domain: 'theater',
    title: 'Just Enough Light',
    leadTemplate: 'Before the show, the {A1} waits on the {A3}, {A2} darkens the room, and the audience is still just a {B3}, a {B2}, and an {B1} waiting to happen.',
    weave: 'A show begins before anyone speaks.',
    note: 'Theater puzzle-id override replaces seats-wait-with phrasing with a pre-show sentence where audience sounds are future energy.',
  },
  'threadline-2027-05-24-campsite': {
    domain: 'campsite',
    title: 'Night Around The Ring',
    leadTemplate: 'Around the ring, the {A1}, {A2}, and {A3} stay close while {B1}, {B2}, and {B3} keep the dark friendly.',
    weave: 'By the fire, open dark has company.',
    note: 'Campsite puzzle-id override removes the warmed-night formula and lets camp gear and shared fire behavior meet in ordinary syntax.',
  },
  'threadline-2027-06-17-apiary': {
    domain: 'apiary',
    title: 'Work In The Air',
    leadTemplate: 'Near the hive, {A1} dusts the {A2}, {A3} warms in the box, and bees {B1} the cells, {B2} in the flowers, and {B3} directions home.',
    weave: 'A hive is a city made of errands.',
    note: 'Apiary puzzle-id override gives tend, forage, and waggle their own bee grammar instead of treating the motion list as filler.',
  },
  'threadline-2027-07-04-tailor': {
    domain: 'tailor',
    title: 'Mirror Before Fit',
    leadTemplate: 'At the fitting mirror, {A1}, {A2}, and {A3} stay under the light while hands {B1}, {B2}, and {B3} the garment.',
    weave: 'Cloth learns the body by small corrections.',
    note: 'Tailor puzzle-id override replaces gathered verb-list grammar with a fitting sentence where each move belongs to hands.',
  },
  'threadline-2027-07-21-airport': {
    domain: 'airport',
    title: 'Distance On Schedule',
    leadTemplate: 'At the gate, the {A1} waits by the {A3}, the {A2} sits beyond the glass, and planes {B1}, flights {B2}, and travelers {B3} bags toward the door.',
    weave: 'The board turns distance into an appointment.',
    note: 'Airport puzzle-id override gives descend, land, and carry separate subjects instead of leaving carry without an object.',
  },
  'threadline-2027-07-10-firehouse': {
    domain: 'firehouse',
    title: 'Red Room Waiting',
    leadTemplate: 'In the bay, {A1}, {A2}, and {A3} stay ready as the crew learns to {B1}, {B2}, and sound the {B3}.',
    weave: 'Practice is why help can hurry.',
    note: 'Firehouse puzzle-id override breaks the repeated before-bell drill structure and gives alert its noun role.',
  },
  'threadline-2027-07-14-clockshop': {
    domain: 'clockshop',
    title: 'Measured Sound',
    leadTemplate: 'At the clockshop counter, {A1}, {A2}, and {A3} sit open while the mechanism learns to {B1}, {B2}, and {B3}.',
    weave: 'Time gets a body before it makes a sound.',
    note: 'Clockshop puzzle-id override removes object/action category grammar and makes the motions belong to the mechanism.',
  },
  'threadline-2027-07-31-planetarium': {
    domain: 'planetarium',
    title: 'A Borrowed Night',
    leadTemplate: 'In the dark room, {A1}, {A2}, and {A3} appear overhead while {B1}, {B2}, and {B3} carry the ceiling away.',
    weave: 'A borrowed night can erase the roof.',
    note: 'Planetarium puzzle-id override replaces move-the-ceiling-outward mechanics with a simpler room-and-ceiling image.',
  },
  'threadline-2027-10-07-planetarium': {
    domain: 'planetarium',
    title: 'The Ceiling Opens',
    leadTemplate: 'The seats tilt toward {A1}, {A2}, and {A3} while {B1}, {B2}, and {B3} carry the ceiling away.',
    weave: 'The dark ceiling teaches the room to travel.',
    note: 'Planetarium puzzle-id override removes uses-list grammar and gives the payoff clear ceiling/travel bridge tokens.',
  },
  'threadline-2027-10-17-planetarium': {
    domain: 'planetarium',
    title: 'The Ceiling Leaves',
    leadTemplate: 'In the dark room, {A1}, {A2}, and {A3} appear overhead while {B1}, {B2}, and {B3} carry the ceiling away.',
    weave: 'The sky comes indoors for one impossible hour.',
    note: 'Planetarium puzzle-id override removes the repeated nonblank dome and keeps the show cues over the seats.',
  },
  'threadline-2027-08-26-lighthouse': {
    domain: 'lighthouse',
    title: 'The Far Warning',
    leadTemplate: 'Above the water, {A1}, {A2}, and {A3} hold the tower\'s work while the keeper learns to {B1}, {B2}, and {B3} the light.',
    weave: 'The kindest light arrives before danger.',
    note: 'Lighthouse puzzle-id override removes light-can chaining and gives the keeper a concrete action sentence.',
  },
  'threadline-2027-08-02-lighthouse': {
    domain: 'lighthouse',
    title: 'The Beam Waits',
    leadTemplate: 'Above the water, {A1}, {A2}, and {A3} face the dark as the beam is set to {B1}, {B2}, and {B3} the warning.',
    weave: 'The coast trusts a light it can read.',
    note: 'Lighthouse puzzle-id override removes repeated keeper wording and lets the beam carry the warning actions.',
  },
  'threadline-2027-08-23-laboratory': {
    domain: 'laboratory',
    title: 'Proof Takes Shape',
    leadTemplate: 'Under the lab light, {A1}, {A2}, and {A3} sit ready while technicians {B1}, {B2}, and {B3} the sample.',
    weave: 'Precision is the poetry of not guessing.',
    note: 'Laboratory puzzle-id override removes result-waits-on phrasing and gives the test verbs a sample object.',
  },
  'threadline-2027-08-25-observatory': {
    domain: 'observatory',
    title: 'Almost Held',
    leadTemplate: 'Under the open roof, {A1}, {A2}, and {A3} steady the view while {B1}, {B2}, and {B3} mark the sky.',
    weave: 'The sky gets closer without getting smaller.',
    note: 'Observatory puzzle-id override removes night-gives phrasing and lets the sky motions mark the view directly.',
  },
  'threadline-2027-08-31-apothecary': {
    domain: 'apothecary',
    title: 'Old Shelf Light',
    leadTemplate: 'On the apothecary shelf, {A1}, {A2}, and {A3} wait while careful hands {B1} herbs, {B2} the dose, and {B3} the remedy.',
    weave: 'A remedy is care made exact.',
    note: 'Apothecary puzzle-id override removes shelf-on-shelf and gather-for-remedy phrasing with specific hand actions.',
  },
  'threadline-2027-09-08-radio-booth': {
    domain: 'radio-booth',
    title: 'Chair Beside The Mic',
    leadTemplate: 'Near the mic, {A1}, {A2}, and {A3} wait while {B1}, {B2}, and {B3} shape the hour.',
    weave: 'Sound becomes public by leaving a small room.',
    note: 'Radio puzzle-id override removes producer-works-through phrasing and lets the air pieces shape the broadcast.',
  },
  'threadline-2027-07-14-newsroom': {
    domain: 'newsroom',
    title: 'Morning Edition',
    leadTemplate: 'At the newsroom desk, the {A1}, {A2}, and {A3} wait while editors {B1} proofs, {B2} sources, and {B3} the margins.',
    weave: 'Truth gets careful before it gets loud.',
    note: 'Newsroom puzzle-id override removes open-out-through-story phrasing and gives print, call, and notate concrete newsroom objects.',
  },
  'threadline-2027-05-15-radio-booth': {
    domain: 'radio-booth',
    title: 'Microphone Warmup',
    leadTemplate: 'In the radio booth, {A1}, {A2}, and {A3} sit near the mic while {B1}, {B2}, and {B3} give the hour its shape.',
    weave: 'A voice leaves the room and keeps someone company.',
    note: 'Radio puzzle-id override removes carry-the-voice-out phrasing and treats bumper, jingle, and stinger as broadcast shape.',
  },
  'threadline-2027-06-01-planetarium': {
    domain: 'planetarium',
    title: 'The Indoor Sky',
    leadTemplate: 'Under the dome, {A1}, {A2}, and {A3} appear overhead while {B1}, {B2}, and {B3} carry the ceiling away.',
    weave: 'The ceiling learns to be far away.',
    note: 'Planetarium puzzle-id override removes show-moves-with wording and keeps the dome/show motion direct.',
  },
  'threadline-2027-06-14-pottery': {
    domain: 'pottery',
    title: 'A Vessel Almost There',
    leadTemplate: 'At the clay table, {A1}, {A2}, and {A3} wait while hands {B1}, {B2}, and {B3} the piece before the kiln.',
    weave: 'The kiln keeps the handprint after the hand is gone.',
    note: 'Pottery puzzle-id override replaces gather-near-kiln grammar with a hands-and-piece sentence.',
  },
  'threadline-2027-06-18-radio-booth': {
    domain: 'radio-booth',
    title: 'Red Lamp Morning',
    leadTemplate: 'In the radio booth, {A1}, {A2}, and {A3} glow near the mic while {B1}, {B2}, and {B3} shape the signal.',
    weave: 'A signal needs a small room first.',
    note: 'Radio puzzle-id override removes broadcast-built-from phrasing and makes the technical moves shape the signal directly.',
  },
  ...
  'threadline-2027-08-15-pottery': {
    domain: 'pottery',
    title: 'Soft Form',
    leadTemplate: 'At the clay table, the {A1}, {A2}, and {A3} wait while hands {B1}, {B2}, and {B3} the piece before the kiln.',
    weave: 'Clay remembers the hand after the fire.',
    note: 'Pottery puzzle-id override removes wheel-on-wheel wording and gives bisque, shard, wheel, and finishing moves a tactile order.',
  },
  'threadline-2027-09-09-dancehall': {
    domain: 'dancehall',
    title: 'The Lights Find Everyone',
    leadTemplate: 'At the dancehall, {A3} hangs above the {A1}, {A2} reaches the floor, and dancers {B1} on the beat, {B2} in place, and {B3} the floor.',
    weave: 'A dance begins when the room agrees.',
    note: 'Dancehall puzzle-id override removes surface/next-motion wording and gives clap, bounce, and cross separate dance grammar.',
  },
  'threadline-2027-09-03-tailor': {
    domain: 'tailor',
    title: 'Cloth Learns Shape',
    leadTemplate: 'At the fitting mirror, {A1}, {A2}, and {A3} lie ready as the {B1} decides where to {B2} and {B3} the garment.',
    weave: 'The mirror lets structure become mercy.',
    note: 'Tailor puzzle-id override removes the repeated tailor noun and puts the person answer in its natural role.',
  },
  'threadline-2027-10-22-printshop': {
    domain: 'printshop',
    title: 'Ink Public',
    leadTemplate: 'In the printshop, {A1}, {A2}, and {A3} wait while workers {B1}, {B2}, and {B3} the page.',
    weave: 'Language gets heavier when it leaves the hand.',
    note: 'Printshop puzzle-id override removes the repeated press anchor and gives staple, press, and collate a page object.',
  },
  'threadline-2027-11-12-printshop': {
    domain: 'printshop',
    title: 'Words Leave The Room',
    leadTemplate: 'In the printshop, {A1}, {A2}, and {A3} pause near the page while workers {B1}, {B2}, and {B3} the order.',
    weave: 'Language gets heavier when it leaves the hand.',
    note: 'Printshop ready-reserve override removes open-the-way-through phrasing and gives proof, stack, and package a shared object.',
  },
  'threadline-2027-11-16-newsroom': {
    domain: 'newsroom',
    title: 'Record Ready',
    leadTemplate: 'At the newsroom desk, {A1}, {A2}, and {A3} stay on the page while editors {B1} the lead, {B2} the proof, and {B3} the record.',
    weave: 'A fact is a rumor that survived the room.',
    note: 'Newsroom ready-reserve override removes move-through-rest phrasing and gives follow, print, and check concrete newsroom objects.',
  },
  'threadline-2027-03-24-desk': {
    domain: 'desk',
    title: 'Plain Light',
    leadTemplate: 'On the desk, {A1}, {A2}, and {A3} leave room for {B1}, {B2}, and {B3}.',
    weave: 'The desk holds the ordinary start of work.',
    note: 'Desk puzzle-id override removes notes-say phrasing and lets the planning nouns sit naturally on the work surface.',
  },
  'threadline-2027-03-25-garden': {
    domain: 'garden',
    title: 'Beds Awake',
    leadTemplate: 'Past the gate, {A1}, {A2}, and {A3} brighten after rain while {B1}, {B2}, and {B3} fill the morning.',
    weave: 'The yard answers quiet work slowly.',
    note: 'Garden puzzle-id override removes make-room-for-work phrasing and gives the tending gerunds a simple after-rain home.',
  },
  'threadline-2027-06-28-pottery': {
    domain: 'pottery',
    title: 'A Vessel Almost There',
    leadTemplate: 'At the clay table, {A1}, {A2}, and {A3} are ready as hands {B1}, {B2}, and {B3} the piece before the kiln.',
    weave: 'Soft clay learns to keep its nerve.',
    note: 'Pottery puzzle-id override removes repeated wheel wording and gives the kiln steps a single piece to work on.',
  },
  'threadline-2027-07-03-pottery': {
    domain: 'pottery',
    title: 'A Vessel Nearly There',
    leadTemplate: 'At the worktable, {A1}, {A2}, and {A3} are ready as hands {B1}, {B2}, and {B3} the piece.',
    weave: 'A vessel remembers the hand after heat.',
    note: 'Pottery puzzle-id override removes repeated shape wording and keeps the finishing verbs on the piece.',
  },
  'threadline-2027-09-21-radio-booth': {
    domain: 'radio-booth',
    title: 'The Hall Hears It',
    leadTemplate: 'In the radio booth, {A1}, {A2}, and {A3} surround the mic while {B1}, {B2}, and {B3} shape the hour.',
    weave: 'The booth stays close while the sound goes outward.',
    note: 'Radio puzzle-id override removes producer-works-through phrasing and treats the air pieces as broadcast shape.',
  },
  'threadline-2027-09-24-campsite': {
    domain: 'campsite',
    title: 'Kindling Hour',
    leadTemplate: 'Near the ring, {A1}, {A2}, and {A3} stay close while {B1}, {B2}, and {B3} settle the camp for night.',
    weave: 'A fire earns the night one task at a time.',
    note: 'Campsite puzzle-id override removes gather/keep-fire-tended scaffolding and gives the gerunds a campsite purpose.',
  },
  ...
  'threadline-2027-09-21-laboratory': {
    ...
    leadTemplate: 'Under the lab light, the {A1}, {A2}, and {A3} sit ready while technicians add a {B1}, {B2} results, and {B3} the change.',
    weave: 'Proof is a small change handled with care.',
    note: 'Laboratory puzzle-id override resolves title and payoff reuse while giving catalyst, report, and track natural technician grammar.',
  },
  'threadline-2027-09-21-apothecary': {
    domain: 'apothecary',
    title: 'Bottle Before Remedy',
    leadTemplate: 'On the apothecary shelf, {A1}, {A2}, and {A3} wait while {B1}, {B2}, and {B3} turn care into a remedy.',
    weave: 'A remedy is patience measured small.',
    note: 'Apothecary puzzle-id override removes gather-for-the-remedy phrasing and gives the gerunds a single care-to-dose action.',
  },
  'threadline-2027-09-09-lighthouse': {
    domain: 'lighthouse',
    title: 'Signal At Dusk',
    leadTemplate: 'Above the water, {A1}, {A2}, and {A3} hold the height while the signal can {B1}, keep {B2}, and {B3} across the dark.',
    weave: 'The coast gets time because light arrives first.',
    note: 'Lighthouse puzzle-id override removes change-the-weight phrasing and gives glance, watch, and outline a direct warning object.',
  },
  'threadline-2027-10-24-lighthouse': {
    domain: 'lighthouse',
    title: 'Bright Above The Rocks',
    leadTemplate: 'Inside the tower, {A1}, {A2}, and {A3} face the dark while the light is set to {B1}, {B2}, and {B3} a warning.',
    weave: 'A light is useful only when danger is near.',
    note: 'Lighthouse puzzle-id override removes repeated stair wording and gives direct, rotate, and steady a single light subject.',
  },
  ...
  'threadline-2027-10-06-newsroom': {
    domain: 'newsroom',
    title: 'Copy Before Dawn',
    leadTemplate: 'At the newsroom desk, the {A1}, {A2}, and {A3} sit under the lamp while reporters keep the story {B1}, {B2} the source, and {B3} the copy.',
    weave: 'A fact is a rumor that survived the room.',
    note: 'Newsroom puzzle-id override removes story-on-story repetition and gives revise a concrete object without overexplaining the fact-check payoff.',
  },
  'threadline-2027-10-07-newsroom': {
    domain: 'newsroom',
    title: 'The Desk Checks Twice',
    leadTemplate: 'At the newsroom desk, {A1}, {A2}, and {A3} wait while editors {B1} for the quote, {B2} the proof, and {B3} the source.',
    weave: 'News becomes public only after care.',
    note: 'Newsroom puzzle-id override removes sit-where-hour-opens phrasing and gives listen, print, and query concrete newsroom objects.',
  },
  ...
  'threadline-2027-10-19-pottery': {
    domain: 'pottery',
    title: 'Hands Before Fire',
    leadTemplate: 'At the pottery wheel, {A1}, {A2}, and {A3} are still ideas while hands {B1}, {B2}, and {B3} the clay.',
    weave: 'Clay remembers pressure after heat.',
    note: 'Pottery puzzle-id override removes duplicated wheel phrasing and makes shape, paddle, and polish belong to hands.',
  },
  'threadline-2027-08-29-pottery': {
    domain: 'pottery',
    title: 'Soft Form Turning',
    leadTemplate: 'At the clay table, {A1}, {A2}, and {A3} wait while hands {B1}, {B2}, and {B3} the piece.',
    weave: 'Heat lets the hand stay visible.',
    note: 'Pottery puzzle-id override removes repeated wheel wording and gives paddle, polish, and center a shared object.',
  },
  'threadline-2027-10-27-apothecary': {
    domain: 'apothecary',
    title: 'Carefully Lit',
    leadTemplate: 'On the apothecary shelf, {A1}, {A2}, and {A3} wait while careful hands {B1}, {B2}, and {B3} the remedy.',
    weave: 'Medicine begins as careful patience.',
    note: 'Apothecary puzzle-id override removes collect-for-the-remedy phrasing and gives pour, strain, and tincture a direct object.',
  },
  'threadline-2027-10-14-apiary': {
    domain: 'apiary',
    title: 'Box In Bloom',
    leadTemplate: 'Near the hive, {A1}, {A2}, and {A3} stay close while bees {B1}, {B2}, and {B3} in the sun.',
    weave: 'The bloom comes home as a hum.',
    note: 'Apiary puzzle-id override removes wake-the-scene phrasing and gives dance, return, and gather a bee subject.',
  },
  'threadline-2027-10-23-weather-station': {
    domain: 'weather-station',
    title: 'Plain Light',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} catch the hour as air can {B1}, screens {B2}, and forecasters {B3}.',
    weave: 'Numbers give the sky a usable shape.',
    note: 'Weather-station puzzle-id override fixes the broken "pressure is freshen" grammar with direct weather subjects.',
  },
  'threadline-2027-10-30-newsroom': {
    domain: 'newsroom',
    title: 'Proof Before Print',
    leadTemplate: 'The {A1}, {A2}, and {A3} stay on the page as editors {B1} the lead, {B2} the story, and keep the copy {B3}.',
    weave: 'News becomes public only after care.',
    note: 'Newsroom puzzle-id override fixes follow/draft/factual grammar with each word in its natural role.',
  },
  'threadline-2027-10-30-apothecary': {
    domain: 'apothecary',
    title: 'The Quiet Remedy',
    leadTemplate: 'On the apothecary shelf, {A1}, {A2}, and {A3} wait while careful hands {B1}, {B2}, and {B3} the remedy.',
    weave: 'A remedy is care made exact.',
    note: 'Apothecary puzzle-id override removes open-out-for-remedy phrasing and gives decoct, infuse, and tincture a direct object.',
  },
  'threadline-2027-10-09-aquarium': {
    domain: 'aquarium',
    title: 'Clear Wall',
    leadTemplate: 'Behind the glass, {A1}, {A2}, and {A3} hold still just long enough for {B1}, {B2}, and {B3} to move around them.',
    weave: 'The glass stays alive because water keeps whispering.',
    note: 'Aquarium puzzle-id override removes gives-room-to phrasing and makes water motion surround the tank details.',
  },
  'threadline-2027-10-09-airport': {
    domain: 'airport',
    title: 'Public Leaving',
    leadTemplate: 'In the departure hall, {A1}, {A2}, and {A3} stay close while screens {B1}, travelers {B2}, and flights {B3}.',
    weave: 'A schedule gives distance a public shape.',
    note: 'Airport puzzle-id override resolves title reuse and gives announce, queue, and connect separate public-room subjects.',
  },
  'threadline-2027-10-12-chessboard': {
    domain: 'chessboard',
    title: 'Next Move',
    leadTemplate: 'At the chess table, {A1}, {A2}, and {A3} sit under pressure while players {B1}, {B2}, and {B3}.',
    weave: 'One patient choice can change the air.',
    note: 'Chessboard puzzle-id override pairs with the castling rejection so this row no longer repeats a root family.',
  },
  'threadline-2027-10-31-chessboard': {
    domain: 'chessboard',
    title: 'The Quiet Square',
    leadTemplate: 'At the chess table, {A1}, {A2}, and {A3} make the position tense while {B1}, {B2}, and {B3} crowd the next move.',
    weave: 'Silence makes the threat bigger.',
    note: 'Chessboard puzzle-id override removes repeated board wording while preserving the position-to-tactic bridge.',
  },
  'threadline-2027-10-15-planetarium': {
    domain: 'planetarium',
    title: 'Seats Under Saturn',
    leadTemplate: 'In the dark room, {A1}, {A2}, and {A3} appear overhead while {B1}, {B2}, and {B3} carry the ceiling away.',
    weave: 'Wonder begins when the room looks up.',
    note: 'Planetarium puzzle-id override removes move-the-ceiling-outward phrasing and keeps the show cues as motion over the seats.',
  },
  ...
  'threadline-2027-09-12-weather-station': {
    domain: 'weather-station',
    title: 'Pressure Before Rain',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} catch the change as pressure can {B1}, wind can {B2}, and readings can {B3}.',
  ...
  'threadline-2027-11-08-weather-station': {
    domain: 'weather-station',
    title: 'Early Notice',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} keep the air legible while forecasters watch it {B1}, {B2}, and {B3}.',
    weave: 'Measurements give bad weather a head start.',
    note: 'Weather-station ready-reserve override removes deepen/settle/freeze list grammar and gives the changes a human observer.',
  },
  'threadline-2027-11-12-weather-station': {
    domain: 'weather-station',
    title: 'Before The Change',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} keep the map honest while the forecast marks a {B1}, a {B2}, and a {B3} by morning.',
    weave: 'A forecast turns surprise into preparation.',
    note: 'Weather-station ready-reserve override removes track-freeze and swing-pressure phrasing by treating the three changes as forecast entries.',
  },
  ...
  'threadline-2027-11-16-radio-booth': {
    ...
    leadTemplate: 'At the booth, {A1}, {A2}, and {A3} sit near the mic for a long night of {B1}, {B2}, and {B3}.',
    weave: 'The booth stays close while the sound goes outward.',
    note: 'Radio-booth ready-reserve override removes cues-plus-gerunds phrasing and makes the air work feel like a long booth shift.',
  },
  ...
  'threadline-2027-11-11-dancehall': {
    ...
    leadTemplate: 'At the dancehall, the {A3} catches {A2} near the {A1} before dancers {B1}, {B2}, and {B3}.',
    weave: 'The beat becomes public when bodies answer.',
    note: 'Dancehall ready-reserve override removes "are in the room before bodies" staging and gives the floor details a visible role.',
  },
  'threadline-2027-11-13-dancehall': {
    domain: 'dancehall',
    title: 'A Public Spark',
    leadTemplate: 'At the dancehall, the {A3} catches {A2} near the {A1} before dancers {B1}, {B2}, and {B3}.',
    weave: 'Music gives a room its pulse.',
    note: 'Dancehall ready-reserve override removes "are in the room before bodies" staging and lets the mirror, ballet, circle, and dancers share one scene.',
  },
  ...
  'threadline-2027-11-17-station': {
    ...
    leadTemplate: 'On the platform, the {A1}, posted {A2}, and the {A3} keep the trip orderly while a {B1}, a {B2}, and {B3} keep the wait bearable.',
    weave: 'A station gives waiting somewhere to point.',
    note: 'Station ready-reserve override makes conduct readable as posted conduct and keeps the waiting habits attached to travelers.',
  },
  ...
  'threadline-2027-11-12-lighthouse': {
    domain: 'lighthouse',
    title: 'Toward The Beam',
    leadTemplate: 'Inside the tower, {A1}, {A2}, and {A3} carry the keeper toward a beam for {B1}, {B2}, and {B3}.',
    weave: 'Height turns a light into warning.',
    note: 'Lighthouse ready-reserve override removes stair-pauses/open-the-way phrasing and makes the tower pieces lead to the beam.',
  },
  'threadline-2027-11-15-clockshop': {
    domain: 'clockshop',
    title: 'Hour Under Glass',
    leadTemplate: 'In the clockshop, {A1}, {A2}, and {A3} lie under glass while the repairer listens for the hour to {B1}, {B2}, and {B3}.',
    weave: 'Time feels close when you can hear it.',
    note: 'Clockshop ready-reserve override removes counter-gathers phrasing and gives advance, turn, and strike an audible hour subject.',
  },
  'threadline-2027-11-16-campsite': {
    domain: 'campsite',
    title: 'The Open Air Settles',
    leadTemplate: 'By the fire, {A1}, {A2}, and {A3} sit close while {B1} catches, {B2} starts, and {B3} keeps supper moving.',
    weave: 'The fire gives outside a center.',
    note: 'Campsite ready-reserve override replaces the gerund-list-before-make structure with separate fire and supper actions.',
  },
  'threadline-2027-11-09-radio-booth': {
    domain: 'radio-booth',
    title: 'A Desk After Dark',
    leadTemplate: 'Behind the glass, {A1}, {A2}, and {A3} sit near the mic as the producer sets {B1}, {B2}, and {B3} in order.',
    weave: 'A small room can keep distance company.',
    note: 'Radio-booth ready-reserve override replaces starts-small abstraction with a concrete booth-to-distance payoff.',
  },
  'threadline-2027-11-19-theater': {
    domain: 'theater',
    title: 'Before The Hush',
    leadTemplate: 'Before the show, {A1}, {A2}, and {A3} hold the stage while {B1}, {B2}, and {B3} gather in the seats.',
    weave: 'The show begins when attention gathers.',
    note: 'Theater ready-reserve override removes lights-find/audience-gathers scaffolding and keeps the reveal at the moment attention forms.',
  },
  'threadline-2027-11-29-theater': {
    domain: 'theater',
    title: 'Still Bright',
    leadTemplate: 'Before the show, {A1}, {A2}, and {A3} hold the stage while the seats fill with {B1}, {B2}, and {B3}.',
    weave: 'The show begins when the room goes quiet.',
    note: 'Theater ready-reserve override removes curtain-rises and house-settles scaffolding while keeping stage details and audience cues natural.',
  },
  'threadline-2027-11-23-shore': {
    domain: 'shore',
    title: 'The Edge Remembers',
    leadTemplate: 'At low tide, {A1}, {A2}, and {A3} stay in the sand while water keeps {B1}, {B2}, and {B3} at the edge.',
    weave: 'The tide edits every beach twice.',
    note: 'Shore ready-reserve override removes before-motion-take-the-edge grammar and makes the water motion a single natural subject.',
  },
  'threadline-2027-11-26-porch': {
    domain: 'porch',
    title: 'Light For Someone Coming',
    leadTemplate: 'On the step, {A1}, {A2}, and {A3} wait in the porch light as {B1}, {B2}, and {B3} come up the walk.',
    weave: 'A porch becomes a place when someone is coming.',
    note: 'Porch ready-reserve override removes step-is-set utility phrasing and puts the neighbor signals on the walk.',
  },
  'threadline-2027-12-03-park': {
    domain: 'park',
    title: 'Early Signal',
    leadTemplate: 'Along the loop, {A1}, {A2}, and {A3} make room for people to {B1}, {B2}, and {B3}.',
    weave: 'The park has a pulse because people come back.',
    note: 'Park ready-reserve override removes familiar-loop-has phrasing and gives the path details a human use.',
  },
  'threadline-2027-11-29-trail': {
    domain: 'trail',
    title: 'The View Opens',
    leadTemplate: 'On the trail, {A1}, {A2}, and {A3} keep the route clear as {B1}, {B2}, and {B3} open around it.',
    weave: 'A clear route makes room for awe.',
    note: 'Trail ready-reserve override replaces the odd Open Hand title and keeps the sign-to-landscape aha concrete.',
  },
  'threadline-2027-12-03-school': {
    domain: 'school',
    title: 'Morning On The Board',
    leadTemplate: 'Before the bell, {A1}, {A2}, and {A3} wait as the morning brings {B1}, {B2}, and {B3}.',
    weave: 'The first lesson is really an agreement.',
    note: 'School ready-reserve override removes board-has list grammar and gives drills, tests, and welcome a morning subject.',
  },
  'threadline-2027-12-03-bakery': {
    domain: 'bakery',
    title: 'Sweet Paper Morning',
    leadTemplate: 'Behind the glass, {A1}, {A2}, and {A3} wait while {B1}, {B2}, and {B3} finish the tray.',
    weave: 'A sweet errand makes morning kinder.',
    note: 'Bakery ready-reserve override removes line-settles-into phrasing and keeps the finishing motions on the tray.',
  },
  'threadline-2027-12-06-porch': {
    domain: 'porch',
    title: 'Expected At The Step',
    leadTemplate: 'At the front walk, {A1}, {A2}, and {A3} hold the house steady as {B1}, {B2}, and {B3} come to the step.',
    weave: 'When someone is expected, even the step looks warm.',
    note: 'Porch ready-reserve override removes can-light-the-step abstraction while keeping the expected-arrival feeling.',
  },
  'threadline-2027-12-07-station': {
    domain: 'station',
    title: 'Kind Light',
    leadTemplate: 'Under the station roof, {A1}, {A2}, and {A3} point the way while travelers find a {B1}, handle {B2}, and turn to a {B3}.',
    weave: 'Station signs keep the wait from drifting.',
    note: 'Station ready-reserve override removes keep-minutes-company phrasing and gives charger, checkin, and doodle believable traveler grammar.',
  },
  'threadline-2027-12-08-workshop': {
    domain: 'workshop',
    title: 'Tool Light',
    leadTemplate: 'Under the bench lamp, {A1}, {A2}, and {A3} wait beside a {B1}, a {B2}, and a {B3} that need attention.',
    weave: 'A repair begins when the problem gets specific.',
    note: 'Workshop ready-reserve override replaces give-the-problem-shape phrasing with direct repair details.',
  },
  'threadline-2027-12-10-laundry': {
    domain: 'laundry',
    title: 'The Folded Hour',
    leadTemplate: 'On laundry day, {A1}, {A2}, and {A3} pass through {B1}, {B2}, and {B3} before the drawer closes.',
    weave: 'Laundry is care you can wear.',
    note: 'Laundry ready-reserve override removes impossible subject/action pairings and uses the wash-day moves as a simple sequence.',
  },
  'threadline-2027-12-13-shore': {
    domain: 'shore',
    title: 'Marks Before Noon',
    leadTemplate: 'Along the foam line, {A1}, {A2}, and {A3} lie in the sand while water keeps {B1}, {B2}, and {B3} at the edge.',
    weave: 'The water leaves a margin, then revises it.',
    note: 'Shore ready-reserve override removes the repeated before-motion-take-the-edge scaffold and keeps the water active at the margin.',
  },
  ...
  'threadline-2027-11-08-printshop': {
    domain: 'printshop',
    title: 'Ink In Daylight',
    leadTemplate: 'At the printshop table, {A1}, {A2}, and {A3} sit ready while printers {B1}, {B2}, and {B3} the page.',
    weave: 'The page gives private words a public body.',
    note: 'Printshop ready-reserve override removes holds-around and open-room phrasing while keeping the type-pieces-to-page reveal concrete.',
  },
  'threadline-2027-11-14-laboratory': {
    domain: 'laboratory',
    title: 'Exact Wonder',
    leadTemplate: 'Under the lab light, the {A1}, {A2}, and {A3} stay close while technicians {B1}, {B2}, and {B3} the sample.',
    weave: 'Wonder learns to be exact.',
    note: 'Laboratory ready-reserve override removes work-continues-with scaffolding and gives record, filter, and culture a technician subject.',
  },
  'threadline-2027-11-20-laundry': {
    domain: 'laundry',
    title: 'Fresh From The Line',
    leadTemplate: 'In the laundry room, {A1}, {A2}, and a {A3} pass through {B1}, {B2}, and {B3} before the line is done.',
    weave: 'Clean things feel ordinary because the work disappears.',
    note: 'Laundry ready-reserve override replaces come-back-from and morning-becomes grammar with a plainer household sentence.',
  },
  'threadline-2027-11-29-mailroom': {
    domain: 'mailroom',
    title: 'Back Room Route',
    leadTemplate: 'In the mailroom, a {A1}, a {A2} slip, and an {A3} land in piles while clerks are {B1}, {B2}, and {B3} the route.',
    weave: "Every doorstep starts as somebody's pile.",
    note: 'Mailroom ready-reserve override removes wait-before and gets-built-from scaffolding while making the route work human and concrete.',
  },
  'threadline-2027-11-30-diner': {
    domain: 'diner',
    title: 'Blue Hour',
    leadTemplate: 'At the booth, {A1}, {A2}, and {A3} sit by the plate while the counter handles a {B1}, {B2}, and {B3}.',
    weave: 'A booth hears the counter before the plate arrives.',
    note: 'Diner ready-reserve override gives request, takeout, and pickup a counter subject instead of making the words travel by themselves.',
  },
  ...
  'threadline-2027-12-02-studio': {
    ...
    leadTemplate: 'At the studio table, the {A1}, {A2}, and {A3} sit near a {B1}, a {B2}, and a {B3} that are not settled yet.',
    weave: 'The first idea starts as a table full of maybes.',
    note: 'Studio ready-reserve override removes wait-under-the-lamp phrasing and lets the marks exist as unsettled draft choices.',
  },
  ...
  'threadline-2027-12-12-garden': {
    domain: 'garden',
    title: 'Past The Gate',
    leadTemplate: 'Past the gate, {A1}, {A2}, and {A3} show what {B1}, {B2}, and {B3} have been protecting.',
    weave: 'A garden is care made visible.',
    note: 'Garden ready-reserve override removes ask-for task grammar and makes the tending visible in the bed itself.',
  },
  'threadline-2027-12-14-gallery': {
    domain: 'gallery',
    title: 'The Long Wall',
    leadTemplate: 'Near the frame, {A1}, {A2}, and {A3} draw visitors into {B1}, {B2}, and {B3} on.',
    weave: 'A wall becomes intimate when someone stops.',
    note: 'Gallery ready-reserve override removes change-the-pace-into phrasing and lets the visitor movement finish naturally with moving on.',
  },
  'threadline-2027-12-15-laundry': {
    domain: 'laundry',
    title: 'Washday Turn',
    leadTemplate: 'The hamper gives up {A1}, {A2}, and {A3}, and the afternoon turns into {B1}, {B2}, and {B3}.',
    weave: 'Laundry gives disorder a second life.',
    note: 'Laundry ready-reserve override replaces the rest-becomes scaffold with a simple chore sentence and a cleaner household payoff.',
  },
  'threadline-2027-12-18-shore': {
    domain: 'shore',
    title: 'Wet Sand Evidence',
    leadTemplate: 'After low tide, {A1}, {A2}, and {A3} stay in the sand as the water keeps {B1}, {B2}, and {B3} away.',
    weave: 'The beach is a note the tide keeps rewriting.',
    note: 'Shore ready-reserve override removes the repeated tide/water/foam is-structure and keeps the motion in one water subject.',
  },
  'threadline-2027-12-19-bakery': {
    domain: 'bakery',
    title: 'Morning Treat',
    leadTemplate: 'Behind the glass, {A1}, {A2}, and {A3} wait for {B1}, {B2}, and {B3} before the box leaves the counter.',
    weave: 'The box carries sweetness past the line.',
    note: 'Bakery ready-reserve override removes already-breakfast phrasing while keeping the treat-to-shop-motion reveal plain.',
  },
  ...
  'threadline-2027-11-19-rooftop': {
    domain: 'rooftop',
    title: 'Rail In Gold',
    leadTemplate: 'From the roof, {A1}, {A2}, and {A3} sit above {B1}, {B2}, and {B3} rising from the street.',
    weave: 'The city loosens when seen from above.',
    note: 'Rooftop ready-reserve override removes the vague less-certain payoff and keeps skyline details tied to the street below.',
  },
  'threadline-2027-11-23-market': {
    domain: 'market',
    title: 'Basket In The Aisle',
    leadTemplate: 'At the market table, {A1}, {A2}, and {A3} catch the eye while shoppers {B1}, {B2}, and {B3}.',
    weave: 'The basket fills where appetite meets choice.',
    note: 'Market ready-reserve override replaces color-becomes-appetite abstraction with a direct appetite-to-choice payoff.',
  },
  'threadline-2027-11-26-picnic': {
    domain: 'picnic',
    title: 'Room On The Grass',
    leadTemplate: 'Under the trees, {A1}, {A2}, and {A3} come out while people {B1}, {B2}, and {B3}.',
    weave: 'Lunch makes a small room in the grass.',
    note: 'Picnic ready-reserve override removes shade-gathers phrasing and keeps the basket-to-company aha literal.',
  },
  'threadline-2027-11-30-harbor': {
    domain: 'harbor',
    title: 'The Quiet Edge',
    leadTemplate: 'At the dock, {A1}, {A2}, and {A3} hold the boat near the work of {B1}, {B2}, and finally {B3}.',
    weave: 'The harbor holds the boat just long enough.',
    note: 'Harbor ready-reserve override removes boat-gets-ready phrasing and keeps the dock-to-departure relationship concise.',
  },
  'threadline-2027-12-03-market': {
    domain: 'market',
    title: 'Under The Stripe',
    leadTemplate: 'Under the striped awning, {A1}, {A2}, and {A3} wait while shoppers {B1}, {B2}, and {B3}.',
    weave: 'The hand learns what appetite already knew.',
    note: 'Market ready-reserve override removes has-list-while phrasing and gives the stall a clearer awning scene.',
  },
  'threadline-2027-12-08-park': {
    domain: 'park',
    title: 'Room In Low Light',
    leadTemplate: 'Around the loop, {A1}, {A2}, and {A3} give people places to {B1}, {B2}, and {B3}.',
    weave: 'A park gives wandering permission.',
    note: 'Park ready-reserve override removes give-the-walk-shape and bench-matters phrasing with a clearer path-to-wandering reveal.',
  },
  'threadline-2027-12-10-diner': {
    domain: 'diner',
    title: 'Late Signal',
    leadTemplate: 'At the booth, {A1}, {A2}, and {A3} sit by the plate while the counter handles a {B1}, an {B2}, and a {B3}.',
    weave: 'A booth feels regular when the counter answers.',
    note: 'Diner ready-reserve override removes booth-has and counter-awake phrasing with a plainer booth/counter sentence.',
  },
  'threadline-2027-12-13-market': {
    domain: 'market',
    title: 'Paper Bag Morning',
    leadTemplate: 'At the stall, {A1}, {A2}, and {A3} fill the crate while shoppers {B1}, {B2}, and {B3}.',
    weave: 'The bag gets heavy one decision at a time.',
    note: 'Market ready-reserve override removes becomes-personal phrasing and makes the bag the concrete payoff.',
  },
  'threadline-2027-12-11-porch': {
    domain: 'porch',
    title: 'Welcome At The Step',
    leadTemplate: 'At the front edge, {A1}, {A2}, and {A3} face the street as {B1}, {B2}, and {B3} turn toward the house.',
    weave: 'The door gives passing a reason to stop.',
    note: 'Porch ready-reserve override removes passing-into-visiting formula and keeps the arrival concrete at the door.',
  },
  'threadline-2027-12-10-porch': {
    domain: 'porch',
    title: 'A House Turned Outward',
    leadTemplate: 'On the step, {A1}, {A2}, and {A3} wait in the porch light as {B1}, {B2}, and {B3} come up the walk.',
    weave: 'A porch becomes a place when someone is coming.',
    note: 'Porch ready-reserve override removes step-is-set and can-light-step phrasing with a concrete arrival sentence.',
  },
  'threadline-2027-12-16-porch': {
    domain: 'porch',
    title: 'A House Turned Outward',
    leadTemplate: 'At the winter door, {A1}, {A2}, and {A3} face the street as {B1}, {B2}, and {B3} come closer.',
    weave: 'A door is social before it opens.',
    note: 'Porch ready-reserve override removes becomes-social phrasing while preserving the outward-facing threshold reveal.',
  },
  'threadline-2027-12-19-theater': {
    domain: 'theater',
    title: 'Open Hand',
    leadTemplate: 'Before the show, {A1}, {A2}, and {A3} hold the stage while the seats fill with {B1}, {B2}, and {B3}.',
    weave: 'The dark learns where to look.',
    note: 'Theater ready-reserve override removes curtain-rises and house-settles scaffolding with a quieter pre-show sentence.',
  },
  'threadline-2027-12-20-rooftop': {
    domain: 'rooftop',
    title: 'Rail In Gold',
    leadTemplate: 'From the roof, {A1}, {A2}, and {A3} hang above {B1}, {B2}, and {B3} below.',
    weave: 'Up high, the city lets the day go.',
    note: 'Rooftop ready-reserve override removes frame-a-city phrasing and keeps the height/evening texture concrete.',
  },
  'threadline-2027-12-21-commute': {
    domain: 'commute',
    title: 'Close Company',
    leadTemplate: 'In the rain, {A1}, {A2}, and {A3} matter before the route reaches {B1}, {B2}, and {B3}.',
    weave: 'A gray morning still knows where to go.',
    note: 'Commute ready-reserve override removes rain-has-the-traveler grammar and keeps wet-weather gear pointed toward the route.',
  },
  'threadline-2027-12-04-mailroom': {
    domain: 'mailroom',
    title: 'Back Room Route',
    leadTemplate: 'In the mailroom, {A1}, {A2}, and {A3} land in piles while {B1}, {B2}, and {B3} turn them into a route.',
    weave: 'Sorting gives distance an address.',
    note: 'Mailroom ready-reserve override removes sorting-shelf and route-takes-shape phrasing with a clearer pile-to-route sentence.',
  },
  'threadline-2027-12-03-mailroom': {
    domain: 'mailroom',
    title: 'Back Room Route',
    leadTemplate: 'In the mailroom, a {A1}, a {A2} slip, and an {A3} land in piles while {B1}, {B2}, and {B3} turn them into a route.',
    weave: 'Sorting gives distance an address.',
    note: 'Mailroom ready-reserve override removes sorting-shelf and route-takes-shape phrasing with a clearer pile-to-route sentence.',
  },
  'threadline-2027-11-29-laundry': {
    domain: 'laundry',
    title: 'A Stack Takes Shape',
    leadTemplate: 'On laundry day, {A1}, {A2}, and {A3} pass through {B1}, {B2}, and {B3} before the drawer closes.',
    weave: 'Care returns as something soft.',
    note: 'Laundry ready-reserve override removes rest-becomes phrasing and uses the wash-day moves as a simple sequence.',
  },
  'threadline-2027-12-03-gallery': {
    domain: 'gallery',
    title: 'A Slower Wall',
    leadTemplate: 'At the gallery wall, {A1}, {A2}, and {A3} hold the room while visitors keep {B1}, {B2}, and {B3}.',
    weave: 'Looking makes a public room feel private.',
    note: 'Gallery ready-reserve override removes attention-becomes phrasing and lets the visitor moves belong to people.',
  },
  'threadline-2027-12-09-bakery': {
    domain: 'bakery',
    title: 'Sweet Paper Morning',
    leadTemplate: 'Behind the glass, {A1}, {A2}, and {A3} wait while {B1}, {B2}, and {B3} finish the tray.',
    weave: 'The box makes the errand gentle.',
    note: 'Bakery ready-reserve override removes line-settles-into phrasing and keeps the finishing motions on the tray.',
  },
  'threadline-2027-12-09-theater': {
    domain: 'theater',
    title: 'Before The Hush',
    leadTemplate: 'Before the show, {A1}, {A2}, and {A3} hold the room while {B1}, {B2}, and {B3} move through the seats.',
    weave: 'The show begins when the seats learn to listen.',
    note: 'Theater ready-reserve override removes room-goes-quiet/audience-gathers scaffolding and gives laugh, giggle, and pause a seat-level sentence.',
  },
  'threadline-2027-12-05-rooftop': {
    domain: 'rooftop',
    title: 'The City Slows Below',
    leadTemplate: 'From the rooftop rail, {A1}, {A2}, and {A3} hold the last light while the street below settles into {B1}, {B2}, and {B3}.',
    weave: 'The city seems kinder from above.',
    note: 'Rooftop ready-reserve override removes rooftop-rail-looks-over phrasing while preserving the softened-city payoff.',
  },
  'threadline-2027-12-09-harbor': {
    domain: 'harbor',
    title: 'Quiet Company',
    leadTemplate: 'At the dock, {A1}, {A2}, and {A3} keep the boat close, with {B1}, {B2}, and {B3} still ahead.',
    weave: 'The dock is where still water starts leaving.',
    note: 'Harbor ready-reserve override removes boat-gets-ready and rail phrasing while keeping dock objects pointed toward departure.',
  },
  'threadline-2027-12-09-rooftop': {
    domain: 'rooftop',
    title: 'A High Quiet',
    leadTemplate: 'From the rooftop rail, {A1}, {A2}, and {A3} hold the last light while the street below settles into {B1}, {B2}, and {B3}.',
    weave: 'Height gives the evening room to soften.',
    note: 'Rooftop ready-reserve override removes rooftop-rail/less-certain abstraction and keeps the evening textures below the rail.',
  },
  'threadline-2027-12-18-market': {
    domain: 'market',
    title: 'The Bag Gets Full',
    leadTemplate: 'At the market table, {A1}, {A2}, and {A3} catch the eye while shoppers {B1}, {B2}, and {B3}.',
    weave: 'Price turns appetite into a decision.',
    note: 'Market ready-reserve override removes market-table-has phrasing and gives the buyer moves a human subject.',
  },
  'threadline-2027-11-23-park': {
    domain: 'park',
    title: 'First Warmth',
    leadTemplate: 'Around the loop, {A1}, {A2}, and {A3} give people places to {B1}, {B2}, and {B3}.',
    weave: 'A park gives wandering permission.',
  ...
  'threadline-2027-12-14-gallery': {
    domain: 'gallery',
    title: 'The Long Wall',
    leadTemplate: 'At the long wall, {A1}, {A2}, and {A3} hold visitors through {B1}, {B2}, and {B3} on.',
    weave: 'A wall becomes intimate when someone stops.',
    note: 'Gallery ready-reserve override smooths the visitor-move gerunds into a single wall sentence.',
  },
  'threadline-2027-12-10-music': {
    domain: 'music',
    title: 'Quiet Proof',
    leadTemplate: 'In the practice room, {A1}, {A2}, and {A3} wait for {B1}, {B2}, and the next {B3}.',
    weave: 'A song starts when everyone shares time.',
    note: 'Music ready-reserve override removes stand-has and gets-musical phrasing with a simpler rehearsal sentence.',
  },
  'threadline-2027-12-10-harbor': {
    domain: 'harbor',
    title: 'The Gentle Edge',
    leadTemplate: 'At the dock, {A1}, {A2}, and {A3} keep the boat close, with {B1}, {B2}, and {B3} still ahead.',
    weave: 'A mooring keeps departure polite.',
    note: 'Harbor ready-reserve override removes rail-gathers phrasing and keeps dock objects pointed toward departure.',
  },
  ...
  'threadline-2027-07-08-vineyard': {
    ...
    leadTemplate: 'In the {A3} row, {A1} catches the sun while the {A2} waits and workers {B1}, {B2}, and {B3} the harvest toward the bottle.',
    weave: 'Wine is weather remembered slowly.',
    note: 'Vineyard puzzle-id override removes nonblank CELLAR repetition and makes taste, ferment, and rack belong to the same harvest sentence.',
  },
  'threadline-2027-10-01-vineyard': {
    domain: 'vineyard',
    title: 'The Slow Taste Starts',
    leadTemplate: 'In the vineyard row, {A1}, {A2}, and {A3} stay close while hands {B1}, {B2}, and {B3} the season.',
    weave: 'The season is saved by becoming flavor.',
    note: 'Vineyard puzzle-id override removes repeated cellar wording and gives prune, bottle, and store one harvest object.',
  },
  'threadline-2027-08-02-lighthouse': {
    domain: 'lighthouse',
    title: 'The Beam Waits',
  ...
  'threadline-2027-08-13-tailor': {
    ...
    weave: 'Cloth gets honest by small correction.',
  ...
  'threadline-2027-10-26-lighthouse': {
    ...
    leadTemplate: 'On the stair, {A1}, {A2}, and {A3} face the dark while the keeper watches the light {B1}, {B2}, and {B3} across the water.',
    weave: 'A warning is kindest before danger is close.',
    note: 'Lighthouse puzzle-id override replaces a spoiler title and gives swivel, glisten, and reflect separate keeper actions.',
  },
  'threadline-2027-10-30-newsroom': {
    domain: 'newsroom',
    title: 'Proof Before Print',
  ...
  'threadline-2026-06-08-gallery': {
    domain: 'gallery',
    title: 'Color In Quiet',
    leadTemplate: 'Under the gallery lamps, {A1}, {A2}, and {A3} hold still while the room fills with {B1}, {B2}, and {B3}.',
    weave: 'A gallery lets attention take its time.',
    note: 'Gallery puzzle-id override retires go-slowly gerund stacking and lets visitor motion read as the room changing around still art.',
  },
  'threadline-2026-06-10-mailroom': {
    domain: 'mailroom',
    title: 'Doorway Route',
    leadTemplate: 'On the sorting shelf, each {A1}, {A2}, and {A3} label waits while clerks begin {B1}, {B2}, and {B3}.',
    weave: 'Every doorstep starts on a sorting shelf.',
    note: 'Mailroom puzzle-id override retires shelf-lists syntax and gives postal a natural label role while keeping the route-to-door payoff concrete.',
  },
  'threadline-2026-06-15-diner': {
    domain: 'diner',
    title: 'Breakfast In Shorthand',
    leadTemplate: 'At the booth, {A1}, {A2}, and {A3} are already familiar when the server hears a {B1}, jots the {B2}, and drops the {B3}.',
    weave: 'A regular booth runs on small replies.',
    note: 'Diner puzzle-id override removes the calls-out request/orders/check list and turns counter talk into a believable server sequence.',
  },
  'threadline-2026-07-27-school': {
    domain: 'school',
    title: 'Room Before Voices',
    leadTemplate: 'Before voices rise, the {A1}, {A2}, and {A3} wait at the front while {B1}, {B2}, and {B3} bring the room awake.',
    weave: 'A classroom begins when quiet tools meet voices.',
    note: 'School puzzle-id override retires share-the-room-with list parking and makes the classroom turn depend on voices entering the tools.',
  },
  'threadline-2026-11-13-harbor': {
    domain: 'harbor',
    title: 'Where The Day Starts',
    leadTemplate: 'At the marina, the {A2} checks the {A1} and {A3} before open water asks for {B1}, {B2}, and {B3}.',
    weave: 'Open water starts with a careful check.',
    note: 'Harbor puzzle-id override removes still-ahead staging and lets pilot, tiller, and compass lead naturally into open-water work.',
  },
  'threadline-2027-01-02-harbor': {
    domain: 'harbor',
    title: 'Marina Awake',
    leadTemplate: 'At the marina, the {A1}, {A2}, and {A3} sit along the dock before open water means {B1}, {B2}, and {B3}.',
    weave: 'Leaving begins before the hull lets go.',
    note: 'Harbor puzzle-id override removes with-still-ahead staging and keeps dock objects connected to the water they precede.',
  },
  'threadline-2027-01-27-harbor': {
    domain: 'harbor',
    title: 'Before The Dock Lets Go',
    leadTemplate: 'By morning, the {A1}, {A2}, and {A3} still hold the boat close before {B1}, {B2}, and {B3} begin.',
    weave: 'The dock turns waiting toward open water.',
    note: 'Harbor puzzle-id override retires morning-boat-beside copy and gives the dock objects a believable hold-before-motion relation.',
  },
  'threadline-2027-02-21-harbor': {
    domain: 'harbor',
    title: 'Still At The Slip',
    leadTemplate: 'At the {A2}, the {A1} and {A3} keep the boat close before {B1}, {B2}, and {B3} carry it out.',
    weave: 'Still water holds the shape of departure.',
    note: 'Harbor puzzle-id override removes rests-beside/still-ahead staging and makes marina a place instead of a loose object.',
  },
  'threadline-2027-05-03-newsroom': {
    domain: 'newsroom',
    title: 'The Desk Checks Twice',
    leadTemplate: 'At the newsroom desk, the {A1}, {A2}, and {A3} are ready as reporters {B1} the detail, {B2} the record, and {B3} the draft.',
    weave: 'The desk turns urgency into trust.',
    note: 'Newsroom puzzle-id override retires gives-room-to/settle-beyond scaffolding and gives notate, verify, and revise concrete newsroom objects.',
  },
  'threadline-2027-06-07-dancehall': {
    domain: 'dancehall',
    title: 'The First Warmth',
    leadTemplate: 'On the dancehall floor, {A1} wait near the {A2}, the {A3} starts up, and dancers {B1}, {B2}, and {B3} into the room.',
    weave: 'The room finds a pulse before anyone speaks.',
    note: 'Dancehall puzzle-id override retires gives-room-to/settle-beyond scaffolding and lets the dance verbs belong to dancers.',
  },
  'threadline-2027-06-27-lighthouse': {
    domain: 'lighthouse',
    title: 'The Warning Climbs',
    leadTemplate: 'Inside the tower, {A1}, {A2}, and {A3} face the dark so the beam can {B1} ships, {B2} the water, and {B3} the coast.',
    weave: 'A far light turns danger into direction.',
    note: 'Lighthouse puzzle-id override retires share-the-warning-with verb parking and gives each coast cue a real beam object.',
  },
  'threadline-2027-06-29-lighthouse': {
    domain: 'lighthouse',
    title: 'Glass For The Fog',
    leadTemplate: 'At the lighthouse stair, the {A1}, {A2}, and {A3} face the fog as the beam can {B1} the rocks, {B2} the water, and {B3} across the coast.',
    weave: 'The warning climbs so the water can read it.',
    note: 'Lighthouse puzzle-id override retires the light-can verb list and gives outline, brighten, and glance separate visual targets.',
  },
  'threadline-2027-06-30-newsroom': {
    domain: 'newsroom',
    title: 'Under Deadline',
    leadTemplate: 'At the newsroom desk, the {A1}, {A2}, and {A3} are ready as editors {B1} the detail, keep the record {B2}, and {B3} the claim.',
    weave: 'A public fact starts as private doubt.',
    note: 'Newsroom puzzle-id override retires gives-room-to/settle-beyond scaffolding and gives confirm, factual, and query ordinary editorial work.',
  },
  'threadline-2027-07-03-tailor': {
    domain: 'tailor',
    title: 'Mirror Before Fit',
    leadTemplate: 'At the tailor mirror, the {A1}, {A2}, and {A3} look unfinished until {B1}, {B2}, and {B3} bring the fit closer.',
    weave: 'The garment becomes personal by degrees.',
    note: 'Tailor puzzle-id override retires gives-room-to/settle-beyond scaffolding and lets the fitting moves improve the garment directly.',
  },
  'threadline-2027-07-06-weather-station': {
    domain: 'weather-station',
    title: 'Sky Accountable',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} catch the change as air can {B1}, forecasts can {B2}, and the needle can {B3}.',
    weave: 'The sky speaks first in instruments.',
    note: 'Weather-station puzzle-id override removes the colon-list forecast scaffold while keeping each weather shift on a natural subject.',
  },
  'threadline-2027-07-27-airport': {
    domain: 'airport',
    title: 'Gate Before Distance',
    leadTemplate: 'At the gate, {A1}, {A2}, and {A3} make the trip official while agents {B1}, travelers {B2}, and machines {B3}.',
    weave: 'Distance gets official at the gate.',
    note: 'Airport puzzle-id override retires terminal-board-beside list parking and gives announce, carry, and scan believable subjects.',
  },
  'threadline-2027-10-04-lighthouse': {
    domain: 'lighthouse',
    title: 'Care Above Water',
    leadTemplate: 'Inside the tower, {A1}, {A2}, and {A3} hold the warning as keepers set the beam to {B1} ships, {B2} through fog, and {B3} the way.',
    weave: 'Light carries care over water.',
    note: 'Lighthouse puzzle-id override retires the light-can verb list and gives warn, flicker, and direct a keeper-controlled signal sequence.',
  },
  'threadline-2027-08-24-radio-booth': {
    domain: 'radio-booth',
    title: 'The Hall Hears It',
    leadTemplate: 'In the radio booth, the {A1} glows, the {A2} warms, and the {A3} listens as {B1}, {B2}, and {B3} shape the hour.',
    weave: 'A small room sends sound farther than walls.',
    note: 'Radio puzzle-id override retires wait-while/producer-works-through scaffolding and turns the booth pieces into a live broadcast moment.',
  },
  'threadline-2027-10-19-campsite': {
    domain: 'campsite',
    title: 'Dark Around The Fire',
    leadTemplate: 'Near the fire, the {A1} hangs by the {A2}, the {A3} is stacked, and after dark there is {B1}, {B2}, and {B3}.',
    weave: 'A little order makes the night kinder.',
    note: 'Campsite puzzle-id override retires close-by/before-motion scaffolding and makes camp gear support a specific after-dark soundscape.',
  },
  'threadline-2027-06-30-firehouse': {
    domain: 'firehouse',
    title: 'Polished Bell',
    leadTemplate: 'In the bay, {A1}, {A2}, and {A3} stay ready so the crew can {B1}, {B2}, and {B3} help without delay.',
    weave: 'Help can hurry because readiness took its time.',
    note: 'Firehouse puzzle-id override retires call-asks-for utility grammar and lets the response verbs belong to the crew.',
  },
  'threadline-2026-12-15-garden': {
    domain: 'garden',
    title: 'The Kept Patch',
    leadTemplate: 'Past the gate, {A1}, {A2}, and {A3} brighten the beds while {B1}, {B2}, and {B3} fill the morning.',
    weave: 'The yard keeps faith with whoever returns.',
    note: 'Garden puzzle-id override retires share-the-morning-with gerund parking and gives the tending motions the morning directly.',
  },
  'threadline-2026-06-10-mailroom': {
    domain: 'mailroom',
    title: 'Parcel Halfway',
    leadTemplate: 'On the sorting shelf, each {A1}, {A2}, and {A3} label waits while clerks begin {B1}, {B2}, and {B3}.',
    weave: 'Every doorstep starts on a sorting shelf.',
    note: 'Mailroom puzzle-id override keeps the shelf-to-door aha while avoiding title reuse and giving postal a natural label role.',
  },
  'threadline-2026-07-27-school': {
    domain: 'school',
    title: 'Bell Waiting',
    leadTemplate: 'Before voices rise, the {A1}, {A2}, and {A3} lie quiet as {B1}, {B2}, and {B3} bring the room awake.',
    weave: 'The bell matters once quiet has company.',
    note: 'School puzzle-id override removes title spoiler tokens and replaces the tool-meets-voices payoff with a shorter classroom aha.',
  },
  'threadline-2027-01-27-harbor': {
    domain: 'harbor',
    title: 'Mooring Light',
    leadTemplate: 'By morning, the {A1}, {A2}, and {A3} still hold the boat close before {B1}, {B2}, and {B3} begin.',
    weave: 'The dock turns waiting toward open water.',
    note: 'Harbor puzzle-id override replaces abstract before-dock titling while keeping the hold-before-motion relation.',
  },
  'threadline-2027-06-30-firehouse': {
    domain: 'firehouse',
    title: 'Quiet Red Light',
    leadTemplate: 'In the bay, {A1}, {A2}, and {A3} stay ready so the crew can {B1} the rig, {B2} the trapped, and {B3} help without delay.',
    weave: 'Help can hurry because readiness took its time.',
    note: 'Firehouse puzzle-id override avoids title reuse and gives each response verb a concrete emergency object.',
  },
  'threadline-2027-07-03-clockshop': {
    domain: 'clockshop',
    title: 'Wound Morning',
    leadTemplate: 'Under the shop light, {A1}, {A2}, and {A3} keep the dial open while a {B1}, a {B2}, and a {B3} give the hour a voice.',
    weave: 'The hour becomes something the room can hold.',
    note: 'Clockshop reserve override removes the repeated mark text and turns time motions into audible dial details.',
  },
  'threadline-2027-07-22-newsroom': {
    domain: 'newsroom',
    title: 'Before Facts Travel',
    leadTemplate: 'At the newsroom desk, the {A1}, {A2}, and {A3} change all night while the staff {B1}, {B2}, and {B3} before dawn.',
    weave: 'Dawn waits for the facts to hold.',
    note: 'Newsroom puzzle-id override avoids close title/payoff reuse while preserving the messy-room-to-trust reveal.',
  },
  'threadline-2027-07-27-airport': {
    domain: 'airport',
    title: 'Gate In Motion',
    leadTemplate: 'At the gate, {A1}, {A2}, and {A3} make the trip official while planes {B1}, agents {B2}, and travelers {B3} bags.',
    weave: 'Distance gets official at the gate.',
    note: 'Airport puzzle-id override avoids title reuse and restores natural subjects for descend, announce, and carry.',
  },
  'threadline-2027-09-07-lighthouse': {
    domain: 'lighthouse',
    title: 'The Beam Waits',
    leadTemplate: 'Inside the tower, {A1}, {A2}, and {A3} face the dark as keepers set the beam to {B1}, {B2}, and {B3} across the water.',
    weave: 'Danger gets a direction from above.',
    note: 'Lighthouse puzzle-id override removes repeated stair text and gives the coast-cue verbs a single visible beam action.',
  },
  'threadline-2027-09-23-tailor': {
    domain: 'tailor',
    title: 'Cloth Learns Shape',
    leadTemplate: 'At the fitting mirror, {A1}, {A2}, and {A3} wait while hands {B1}, {B2}, and {B3} the cloth.',
    weave: 'The body teaches the garment its shape.',
    note: 'Tailor puzzle-id override removes repeated tailor text and makes the fitting moves belong to hands.',
  },
  'threadline-2027-10-06-newsroom': {
    domain: 'newsroom',
    title: 'Proof Near Dawn',
    leadTemplate: 'At the newsroom desk, the {A1}, {A2}, and {A3} sit under the lamp while reporters {B1} for the quote, {B2} the source, and {B3} the copy.',
    weave: 'A fact is a rumor that survived the room.',
    note: 'Newsroom puzzle-id override avoids title reuse, removes repeated story text, and gives listen, confirm, and query concrete reporting objects.',
  },
  'threadline-2027-10-08-pottery': {
    domain: 'pottery',
    title: 'Soft Form',
    leadTemplate: 'At the studio table, {A1}, {A2}, and {A3} sit near the kiln while hands {B1}, {B2}, and {B3} the clay.',
    weave: 'The finished shape still knows the hand.',
    note: 'Pottery reserve override removes repeated wheel text and gives center, pinch, and paddle a direct clay object.',
  },
  'threadline-2027-10-23-weather-station': {
    domain: 'weather-station',
    title: 'Early Reading',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} catch the hour as air can {B1}, wind can {B2}, and the forecast can {B3}.',
    weave: 'Numbers give the sky a usable shape.',
    note: 'Weather-station puzzle-id override avoids title reuse and gives each forecast-shift verb a believable weather subject.',
  },
  'threadline-2027-09-25-tailor': {
    domain: 'tailor',
    title: 'Mirror With Pins',
    leadTemplate: 'At the fitting mirror, {A1}, {A2}, and {A3} guide the hands as they {B1}, {B2}, and {B3}.',
    weave: 'A better fit is a quiet agreement with the body.',
    note: 'Tailor puzzle-id override keeps the natural hands-and-cloth lead while replacing a close reused mirror payoff.',
  },
  'threadline-2026-05-10-market': {
    domain: 'market',
    title: 'Coins At The Counter',
    leadTemplate: 'Shoppers pause over {A1}, {A2}, and {A3} before they {B1}, {B2}, and {B3}.',
    weave: 'At a stall, appetite becomes a hand.',
    note: 'Market puzzle-id override retires the has-to-point payoff while keeping the stall-choice aha short and human.',
  },
  'threadline-2026-05-28-desk': {
    domain: 'desk',
    title: 'Open Notebook',
    leadTemplate: 'At the desk, {A1}, {A2}, and {A3} hold the morning steady while {B1}, {B2}, and {B3} come into order.',
    weave: 'A clear surface quiets the first task.',
    note: 'Desk puzzle-id override retires first-handle copy and replaces morning-narrows syntax with a calmer desk sentence.',
  },
  'threadline-2026-05-31-kitchen': {
    domain: 'kitchen',
    title: 'Heat Coming',
    leadTemplate: 'On the counter, {A1}, {A2}, and {A3} meet the {B1}, {B2}, and {B3} that make supper.',
    weave: 'Dinner starts when heat finds the counter.',
    note: 'Kitchen puzzle-id override removes raw-things payoff language and leaves the aha on heat meeting counter work.',
  },
  'threadline-2026-07-11-harbor': {
    domain: 'harbor',
    title: 'Harbor In The Bell',
    leadTemplate: 'At the dock, {A1}, {A2}, and {A3} hold the boat until {B1}, {B2}, and {B3} begin on the water.',
    weave: 'The dock keeps departure from becoming sudden.',
    note: 'Harbor puzzle-id override retires still-tied-near/waiting-on-water staging and keeps departure held by dock details.',
  },
  'threadline-2026-08-29-diner': {
    domain: 'diner',
    title: 'Another Cup',
    leadTemplate: 'The booth feels familiar around {A1}, {A2}, and {A3} while the server hears {B1}, {B2}, and {B3}.',
    weave: 'A booth feels regular when the counter answers.',
    note: 'Diner puzzle-id override retires counter-stays-busy list copy and turns the counter talk into something a server hears.',
  },
  'threadline-2026-08-30-harbor': {
    domain: 'harbor',
    title: 'Morning At The Mooring',
    leadTemplate: 'At the {A2}, the {A1} and {A3} keep the boat close before {B1}, {B2}, and {B3} begin on the water.',
    weave: 'Still water waits until distance calls.',
    note: 'Harbor puzzle-id override retires still-tied-near/waiting-on-water staging while letting marina serve as the place.',
  },
  'threadline-2026-09-02-picnic': {
    domain: 'picnic',
    title: 'Noon Without Walls',
    leadTemplate: 'By the trees, {A1}, {A2}, and {A3} are passed around as people {B1}, {B2}, and {B3}.',
    weave: 'A shared meal teaches the park to stay.',
    note: 'Picnic puzzle-id override retires stay-close-while parking and gives the food a lived passing-around action.',
  },
  'threadline-2026-09-05-desk': {
    domain: 'desk',
    title: 'Clean Start',
    leadTemplate: 'Before work begins, the {A1}, {A2}, and {A3} give the morning a place for {B1}, {B2}, and {B3}.',
    weave: 'A clear desk lets the day begin.',
    note: 'Desk puzzle-id override replaces gets-specific payoff language with a simpler surface-to-start aha.',
  },
  'threadline-2026-09-08-kitchen': {
    domain: 'kitchen',
    title: 'A Little More Salt',
    leadTemplate: 'Before the pan is hot, {A1}, {A2}, and {A3} meet {B1}, {B2}, and {B3} on the counter.',
    weave: 'The counter turns hunger toward dinner.',
    note: 'Kitchen puzzle-id override retires ready-for-gerunds copy and gives the ingredients a direct counter action.',
  },
  'threadline-2026-09-27-picnic': {
    domain: 'picnic',
    title: 'Blanket Table',
    leadTemplate: 'Under the trees, {A1}, {A2}, and {A3} spread out while people {B1}, {B2}, and {B3}.',
    weave: 'Lunch makes a room out of open air.',
    note: 'Picnic puzzle-id override retires are-enough-for-people syntax and keeps the blanket scene simple.',
  },
  'threadline-2026-11-12-diner': {
    domain: 'diner',
    title: 'Regular Seat',
    leadTemplate: 'The booth feels familiar around {A1}, {A2}, and {A3} while the counter answers with {B1}, {B2}, and {B3}.',
    weave: 'A regular booth needs the counter voice.',
    note: 'Diner puzzle-id override retires counter-stays-busy list copy and makes the second thread audible from the counter.',
  },
  'threadline-2027-01-05-picnic': {
    domain: 'picnic',
    title: 'Under The Trees',
    leadTemplate: 'Under one tree, {A1}, {A2}, and {A3} spread across lunch while people {B1}, {B2}, and {B3}.',
    weave: 'Lunch turns the park into a table.',
    note: 'Picnic puzzle-id override retires are-enough-for-people syntax and lets the packed items open the lunch scene.',
  },
  'threadline-2027-01-11-kitchen': {
    domain: 'kitchen',
    title: 'Recipe Motion',
    leadTemplate: 'Before the pan heats, {A1}, {A2}, and {A3} meet {B1}, {B2}, and {B3} on the counter.',
    weave: 'The room smells real before dinner has a name.',
    note: 'Kitchen puzzle-id override retires ready-for-gerunds copy and keeps the prep words grounded on the counter.',
  },
  'threadline-2027-05-24-campsite': {
    domain: 'campsite',
    title: 'Night Around The Ring',
    leadTemplate: 'Around the ring, {A1}, {A2}, and {A3} catch the firelight as {B1}, {B2}, and {B3} keep the dark friendly.',
    weave: 'By the fire, open dark has company.',
    note: 'Campsite puzzle-id override retires stay-close-while staging and lets the camp motions carry the dark.',
  },
  'threadline-2027-06-16-apothecary': {
    domain: 'apothecary',
    title: 'Measured Care',
    leadTemplate: 'At the apothecary shelf, {A1}, {A2}, and {A3} wait while careful hands {B1}, {B2}, and {B3}.',
    weave: 'A remedy begins as patience measured small.',
    note: 'Apothecary puzzle-id override retires open-out-for and quiet-purpose copy while giving each remedy verb a hand subject.',
  },
  'threadline-2027-06-25-observatory': {
    domain: 'observatory',
    title: 'Far Light Indoors',
    leadTemplate: 'Under the open roof, {A1}, {A2}, and {A3} hold still while {B1}, {B2}, and {B3} redraw the night.',
    weave: 'The sky comes close by remaining far.',
    note: 'Observatory puzzle-id override retires ready-for-gerunds copy and makes the sky motions act on the night.',
  },
  'threadline-2027-07-07-newsroom': {
    domain: 'newsroom',
    title: 'Morning Needs Proof',
    leadTemplate: 'At the newsroom desk, {A1}, {A2}, and {A3} sit under the lamp while reporters {B1} the claim, {B2} the proof, and {B3} the correction.',
    weave: 'A messy room can still protect the facts.',
    note: 'Newsroom puzzle-id override retires busy-on-desk/move-through-story copy and gives each press move a concrete object.',
  },
  'threadline-2027-07-13-newsroom': {
    domain: 'newsroom',
    title: 'Copy Before Dawn',
    leadTemplate: 'At the newsroom desk, {A1}, {A2}, and {A3} sit under the lamp while editors {B1} the source, {B2} the desk, and {B3} the record.',
    weave: 'Truth gets careful before it gets loud.',
    note: 'Newsroom puzzle-id override retires visible-part/rest staging and turns check, call, and verify into ordinary editor work.',
  },
  'threadline-2027-07-17-newsroom': {
    domain: 'newsroom',
    title: 'Morning Edition',
    leadTemplate: 'At the newsroom desk, {A1}, {A2}, and {A3} sit under the lamp while reporters {B1} the draft, {B2} the claim, and {B3} the record.',
    weave: 'A checked story earns daylight.',
    note: 'Newsroom puzzle-id override retires keeps-company-with and gather-through-story staging in favor of concrete reporting objects.',
  },
  'threadline-2027-07-25-pottery': {
    domain: 'pottery',
    title: 'Soft Form Turning',
    leadTemplate: 'At the wheel, {A1}, {A2}, and {A3} wait under the lamp while hands {B1}, {B2}, and {B3} the piece.',
    weave: 'Heat keeps the hand in the finished shape.',
    note: 'Pottery puzzle-id override retires keeps-company-with/gather-near-kiln staging and makes the finishing verbs belong to hands.',
  },
  'threadline-2027-09-06-airport': {
    domain: 'airport',
    title: 'Long Room',
    leadTemplate: 'In the long room, {A1}, {A2}, and {A3} point outward as planes {B1}, travelers {B2}, and pilots {B3}.',
    weave: 'Distance becomes official before it becomes sky.',
    note: 'Airport puzzle-id override retires stay-close-while/open-out phrasing and gives flight verbs natural subjects.',
  },
  'threadline-2027-09-17-vineyard': {
    domain: 'vineyard',
    title: 'The Slow Taste Starts',
    leadTemplate: 'In the vineyard row, {A1}, {A2}, and {A3} hold the season while workers {B1}, {B2}, and {B3}.',
    weave: 'A bottle begins before the fruit leaves the row.',
    note: 'Vineyard puzzle-id override retires visible-part/rest staging and makes the cellar steps part of vineyard labor.',
  },
  'threadline-2027-09-21-chessboard': {
    domain: 'chessboard',
    title: 'Pressure In The Corner',
    leadTemplate: 'At the board, {A1}, {A2}, and {A3} sit quiet while a player learns to {B1}, {B2}, and {B3}.',
    weave: 'A quiet board can raise the pulse.',
    note: 'Chessboard puzzle-id override retires wait-nearby list parking and lands the board/tactics reveal at room level.',
  },
  'threadline-2027-10-09-chessboard': {
    domain: 'chessboard',
    title: 'Black And White',
    leadTemplate: 'On the board, {A1}, {A2}, and {A3} sit quiet while a player learns to {B1}, {B2}, and {B3}.',
    weave: 'Danger arrives before a piece is touched.',
    note: 'Chessboard puzzle-id override retires visible-part/rest staging and gives the tactics a player subject.',
  },
  'threadline-2027-10-14-apiary': {
    domain: 'apiary',
    title: 'Box In Bloom',
    leadTemplate: 'Near the hive, {A1} and {A2} draw the {A3} back as bees {B1}, {B2}, and {B3} in the sun.',
    weave: 'The bloom comes home as a hum.',
    note: 'Apiary puzzle-id override retires stay-close-while staging and lets the bees answer the hive details.',
  },
  'threadline-2027-10-29-aquarium': {
    domain: 'aquarium',
    title: 'Small Blue World',
    leadTemplate: 'Behind the glass, {A1}, {A2}, and {A3} hover while {B1}, {B2}, and {B3} keep the water awake.',
    weave: 'The tank stays alive by never holding still.',
    note: 'Aquarium puzzle-id override retires keeps-company-with/gather-in-water staging and makes water motion support the living tank.',
  },
  'threadline-2027-07-25-pottery': {
    domain: 'pottery',
    title: 'Soft Form Turning',
    leadTemplate: 'At the studio table, {A1}, {A2}, and {A3} wait under the lamp while hands {B1}, {B2}, and {B3} the piece.',
    weave: 'Heat keeps the hand in the finished shape.',
    note: 'Pottery puzzle-id override keeps the hand/heat aha while avoiding a repeated wheel answer in the filled lead.',
  },
  'threadline-2027-10-29-newsroom': {
    domain: 'newsroom',
    title: 'The Desk Checks Twice',
    leadTemplate: 'On the newsroom desk, {A1}, {A2}, and {A3} stay under the lamp as reporters {B1} the lead, {B2} for the name, and keep the record {B3}.',
    weave: 'News earns trust before it gets loud.',
    note: 'Newsroom puzzle-id override removes repeated story text and gives follow, listen, and factual concrete reporting roles.',
  },
  'threadline-2026-09-27-picnic': {
    domain: 'picnic',
    title: 'Blanket Table',
    leadTemplate: 'Under the trees, {A1}, {A2}, and {A3} pass from hand to hand while people {B1}, {B2}, and {B3}.',
    weave: 'Lunch makes a room out of open air.',
    note: 'Picnic reserve override replaces spread-out phrasing with a more human passing-around sentence.',
  },
  'threadline-2026-11-12-diner': {
    domain: 'diner',
    title: 'Regular Seat',
    leadTemplate: 'The booth feels familiar around {A1}, {A2}, and {A3} while the server brings a {B1}, hears a {B2}, and writes the {B3}.',
    weave: 'A regular booth needs the counter voice.',
    note: 'Diner reserve override removes answers-with scaffolding and gives refill, sizzle, and order a natural service sequence.',
  },
  'threadline-2026-12-08-harbor': {
    domain: 'harbor',
    title: 'Marina Awake',
    leadTemplate: 'At the dock, {A1}, {A2}, and {A3} hold the boat before {B1}, {B2}, and {B3} begin on the water.',
    weave: 'A dock is a held breath before open water.',
    note: 'Harbor reserve override retires still-tied-near/waiting-on-water staging while keeping the dock-to-departure image.',
  },
  'threadline-2026-12-14-desk': {
    domain: 'desk',
    title: 'A Place To Begin',
    leadTemplate: 'At the desk, {A1}, {A2}, and {A3} hold the morning steady while {B1}, {B2}, and {B3} come into order.',
    weave: 'A clear surface quiets the first task.',
    note: 'Desk reserve override removes first-handle payoff copy and replaces morning-narrows syntax.',
  },
  'threadline-2027-03-02-kitchen': {
    domain: 'kitchen',
    title: 'First Flavor',
    leadTemplate: 'Before the pan is hot, {A1}, {A2}, and {A3} meet {B1}, {B2}, and {B3} on the counter.',
    weave: 'The first promise of supper is smell.',
    note: 'Kitchen reserve override retires ready-for-gerunds copy and grounds the prep moves on the counter.',
  },
  'threadline-2027-04-21-kitchen': {
    domain: 'kitchen',
    title: 'Supper Signal',
    leadTemplate: 'Before the pan is hot, {A1}, {A2}, and {A3} meet {B1}, {B2}, and {B3} on the counter.',
    weave: 'The counter wakes before the pan speaks.',
    note: 'Kitchen reserve override retires ready-for-gerunds copy and gives the ingredients a direct counter action.',
  },
  'threadline-2027-07-06-airport': {
    domain: 'airport',
    title: 'Altitude Waiting',
    leadTemplate: 'At the terminal window, {A1}, {A2}, and {A3} frame the trip while agents {B1} bags, travelers {B2}, and planes {B3}.',
    weave: 'Departure makes the room wider before anyone leaves.',
    note: 'Airport reserve override removes easy-light/gathering-toward-gate staging and gives each flight move a natural subject.',
  },
  'threadline-2027-07-26-pottery': {
    domain: 'pottery',
    title: 'Pressure At The Wheel',
    leadTemplate: 'At the studio table, {A1}, {A2}, and {A3} take shape as {B1}, {B2}, and {B3} finish the piece.',
    weave: 'A vessel remembers pressure after heat.',
    note: 'Pottery reserve override replaces broken hands/firing grammar and removes repeated clay text.',
  },
  'threadline-2027-09-10-radio-booth': {
    domain: 'radio-booth',
    title: 'The Desk Lamp Waits',
    leadTemplate: 'In the radio booth, {A1}, {A2}, and {A3} sit near the mic as {B1}, {B2}, and {B3} shape the hour.',
    weave: 'Sound leaves with the room still around it.',
    note: 'Radio reserve override retires producer-works-through staging and lets the air moves shape the hour directly.',
  },
  'threadline-2027-11-08-weather-station': {
    domain: 'weather-station',
    title: 'Early Notice',
    leadTemplate: 'At the weather desk, {A1}, {A2}, and {A3} keep the air legible as pressure can {B1}, rain keeps {B2}, and forecasters {B3}.',
    weave: 'Measurements give bad weather a head start.',
    note: 'Weather-station reserve override removes repeated watch text and makes settle, falling, and watch fit natural subjects.',
  },
  'threadline-2027-10-09-chessboard': {
    domain: 'chessboard',
    title: 'Corner Pressure',
    leadTemplate: 'On the board, {A1}, {A2}, and {A3} sit quiet while a player learns to {B1}, {B2}, and {B3}.',
    weave: 'The room gets quieter when danger is possible.',
    note: 'Chessboard puzzle-id override avoids title/payoff reuse while keeping tactics attached to a player.',
  },
  'threadline-2027-09-21-radio-booth': {
    domain: 'radio-booth',
    ...
    leadTemplate: 'In the radio booth, {A1}, {A2}, and {A3} surround the mic while {B1}, {B2}, and {B3} shape the hour.',
    weave: 'The booth stays close while the sound goes outward.',
    note: 'Radio puzzle-id override avoids title reuse while preserving the booth-to-air movement.',
  },
  'threadline-2027-10-30-newsroom': {
    domain: 'newsroom',
    title: 'Proof Before Print',
    leadTemplate: 'At the newsroom desk, {A1}, {A2}, and {A3} stay on the page as editors {B1} the lead, {B2} the source, and {B3} the copy.',
    weave: 'A public fact leaves only after the room is sure.',
    note: 'Newsroom puzzle-id override replaces repeated public-after-care payoff and fixes the update/edit/publish objects.',
  },
  'threadline-2027-01-13-library': {
    domain: 'library',
    title: 'One Chair Waiting',
    leadTemplate: 'The page has {A1}, {A2}, and {A3}, and the quiet around it leaves space for {B1}, {B2}, and {B3}.',
    weave: 'The shelf comes closer when the room lowers its voice.',
    note: 'Library puzzle-id override breaks an overused a-low-voice-can payoff skeleton while keeping the shelf/quiet-room aha.',
  },
  'threadline-2026-12-14-desk': {
    domain: 'desk',
    title: 'A Place To Begin',
    leadTemplate: 'At the desk, {A1}, {A2}, and {A3} hold the morning steady while {B1}, {B2}, and {B3} come into order.',
    weave: 'Order gives the morning somewhere to land.',
    note: 'Desk reserve override avoids close payoff reuse with Open Notebook while keeping the prepared-desk aha.',
  },
  'threadline-2026-09-08-kitchen': {
    domain: 'kitchen',
    title: 'A Little More Salt',
    leadTemplate: 'On the counter, {A1}, {A2}, and {A3} meet {B1}, {B2}, and {B3} before the pan is hot.',
    weave: 'The counter turns hunger toward dinner.',
    note: 'Kitchen puzzle-id override varies the ready-for-gerunds replacement so the schedule does not form a new lead cluster.',
  },
  'threadline-2027-03-02-kitchen': {
    domain: 'kitchen',
    title: 'First Flavor',
    leadTemplate: 'Before first heat, {A1}, {A2}, and {A3} meet {B1}, {B2}, and {B3}.',
    weave: 'The first promise of supper is smell.',
    note: 'Kitchen reserve override varies the counter-meets-prep structure and keeps the ingredients ahead of heat.',
  },
  'threadline-2027-04-21-kitchen': {
    domain: 'kitchen',
    title: 'Supper Signal',
    leadTemplate: 'Before supper warms, {A1}, {A2}, and {A3} meet {B1}, {B2}, and {B3}.',
    weave: 'The counter wakes before the pan speaks.',
    note: 'Kitchen reserve override varies the counter-meets-prep structure and avoids the ready-for-gerunds scaffold.',
  },
  'threadline-2027-07-06-airport': {
    domain: 'airport',
    title: 'Altitude Waiting',
    leadTemplate: 'At the terminal, {A1} and {A2} clear the {A3} before agents {B1} bags, travelers {B2}, and planes {B3}.',
    weave: 'Departure makes the room wider before anyone leaves.',
    note: 'Airport reserve override removes frame-the-trip staging while giving check, connect, and cruise natural subjects.',
  },
};
...
export function isThreadlineMechanicalWeave(weave: string): boolean {
  return /^(between|by the time|what starts with|with)\b/i.test(weave.trim()) ||
    /\b(has to point|first handle on work|raw things|quiet purpose|gets specific|glass stays alive because water keeps whispering|shelf has a quiet purpose)\b/i.test(weave) ||
...
export function isThreadlineRoboticLead(completedLead: string): boolean {
  return THREADLINE_RECENTLY_RETIRED_LEAD_COPY.test(completedLead) ||
    /\b(keeps company with|open out for|wait nearby|visible part|through the rest|busy on the desk|move through the story|still tied near [^.?!]+ waiting on the water|are enough for people to|are ready for [a-z]+ing, [a-z]+ing, and [a-z]+ing|stays? close while|stays? busy with|open out toward|gather through the story|gather near the kiln)\b/i.test(completedLead) ||
  ...
  if (context.domain === 'harbor') {
    return leadFromTemplate(
      pickLeadTemplate(context, [
        ...
        'Along the dock, {A} catch the light before {B} take the boat out.',
        'The boat is still tied near {A} with {B} waiting on the water.',
        'By the dock, {A} keep the boat close before {B} carry it out.',
        'At the marina, the boat rests near {A} before {B} begin on the water.',
        'Before the boat pulls away, {A} stay close and {B} wait beyond the slips.',
        'The crew checks {A} while the morning boat gets ready for {B}.',
        'The dock holds {A} before the water is ready for {B}.',
