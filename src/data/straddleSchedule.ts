import type {
  StraddleCategory,
  StraddlePuzzleDefinition,
  StraddleTile,
  StraddleTileId,
} from "./straddlePrototype";

export const STRADDLE_SCHEDULE_START_DATE = "2026-05-15";
export const STRADDLE_SCHEDULE_DAYS = 365;
export const STRADDLE_SCHEDULE_VERSION = "v1";
export const STRADDLE_WORD_REUSE_SOFT_COOLDOWN_DAYS = 21;
export const STRADDLE_MAX_WORD_REUSE_TARGET = 18;

export type StraddleScheduleDifficulty = "easy" | "medium" | "hard";
export type StraddleScheduleLane =
  | "concrete"
  | "modern"
  | "word-form"
  | "phrase"
...
export interface StraddleScheduleCategory {
  label: string;
  words: string[];
}

export interface StraddleScheduledPuzzle {
  id: string;
  date: string;
  dayIndex: number;
  difficulty: StraddleScheduleDifficulty;
  editorialLane: StraddleScheduleLane;
  theme: string;
  familyId: string;
  centerWord: string;
  rows: StraddleScheduleCategory[];
  columns: StraddleScheduleCategory[];
  grid: string[][];
  pillarWord?: string;
  holiday?: {
    name: string;
...
export interface StraddleScheduleEditorialAudit {
  difficultyCounts: Record<StraddleScheduleDifficulty, number>;
  laneCounts: Record<StraddleScheduleLane, number>;
  satisfaction: {
    averageScore: number;
    minimumScore: number;
  };
...
interface StraddleScheduleRowSeed {
  label: string;
  words: [string, string, string];
}

interface StraddleScheduleFamily {
  id: string;
  theme: string;
  editorialLane: StraddleScheduleLane;
  columns: [string, string, string];
  rows: StraddleScheduleRowSeed[];
}

interface StraddleHolidaySeed {
  date: string;
  name: string;
  difficulty: StraddleScheduleDifficulty;
  columns: [string, string, string];
  rows: [
    StraddleScheduleRowSeed,
    StraddleScheduleRowSeed,
    StraddleScheduleRowSeed,
  ];
}

interface StraddleSpecialSeed {
  date: string;
  familyId: string;
  theme: string;
  editorialLane: StraddleScheduleLane;
...
interface StraddleScheduleBuildState {
...
const DIFFICULTY_CADENCE: StraddleScheduleDifficulty[] = [
  "medium",
  "hard",
  "medium",
  "easy",
...
export const STRADDLE_SCHEDULE_FAMILIES: StraddleScheduleFamily[] = [
  {
    id: "starts-bcs",
...
const STRADDLE_HOLIDAY_SEEDS: StraddleHolidaySeed[] = [
  {
    date: "2027-05-09",
    name: "Mother's Day",
    difficulty: "easy",
...
export const STRADDLE_HOLIDAYS = STRADDLE_HOLIDAY_SEEDS.map((holiday) => ({
  date: holiday.date,
  name: holiday.name,
}));

const STRADDLE_PILLAR_SEEDS: StraddleSpecialSeed[] = [
  {
    date: "2026-11-03",
...
const SPECIAL_BY_DATE = new Map<string, StraddleSpecialSeed>([
  ...STRADDLE_HOLIDAY_SEEDS.map((holiday): [string, StraddleSpecialSeed] => [
    holiday.date,
    {
      date: holiday.date,
  ...
  ...STRADDLE_PILLAR_SEEDS.map((seed): [string, StraddleSpecialSeed] => [
    seed.date,
    seed,
  ]),
]);
...
function createTileId(word: string, index: number): StraddleTileId {
  return `${slugify(word)}-${index}`;
}

function createFallbackUnsolvedBoard(
  solutionBoard: StraddleTileId[],
): StraddleTileId[] {
  const fallbackPattern = [0, 5, 8, 1, 4, 6, 2, 7, 3];
  return fallbackPattern.map((index) => solutionBoard[index]);
}

export function createStraddlePuzzleDefinitionFromScheduledPuzzle(
  puzzle: StraddleScheduledPuzzle,
): StraddlePuzzleDefinition {
  const words = puzzle.grid.flat();
  const idsByWord = new Map<string, StraddleTileId>(
    words.map((word, index) => [word, createTileId(word, index)]),
  );
  const tiles: StraddleTile[] = words.map((word) => ({
  ...
  const rowCategories: StraddleCategory[] = puzzle.rows.map((row, index) => ({
  ...
  const columnCategories: StraddleCategory[] = puzzle.columns.map(
    (column, index) => ({
...
function hasUniqueWords(rows: StraddleScheduleRowSeed[]): boolean {
  const words = rows.flatMap((row) => row.words);
  return new Set(words).size === words.length;
}

function buildRowCombinations(
  family: StraddleScheduleFamily,
): StraddleScheduleRowSeed[][] {
  const combinations: StraddleScheduleRowSeed[][] = [];
...
function buildSpecialPuzzleForDay(
  seed: StraddleSpecialSeed,
  dayIndex: number,
): StraddleScheduledPuzzle {
  const grid = seed.rows.map((row) => [...row.words]);
  const centerWord = grid[1][1];
  const puzzle: StraddleScheduledPuzzle = {
    id: `straddle-${seed.date}-${seed.familyId}`,
    date: seed.date,
    dayIndex,
...
function registerPuzzle(
  state: StraddleScheduleBuildState,
  puzzle: StraddleScheduledPuzzle,
): void {
...
function scoreRowsForSchedule(
  ...
  words.forEach((word) => {
    ...
    if (lastSeen !== undefined) {
      ...
      if (gap < STRADDLE_WORD_REUSE_SOFT_COOLDOWN_DAYS) {
        score += (STRADDLE_WORD_REUSE_SOFT_COOLDOWN_DAYS - gap) * 120;
      }
    }
  });
...
function buildGenericPuzzleForDay(
  ...
  const familyIndex = dayIndex % STRADDLE_SCHEDULE_FAMILIES.length;
  const family = STRADDLE_SCHEDULE_FAMILIES[familyIndex];
  ...
  return {
    id: `straddle-${date}-${family.id}-${slugify(rows.map((row) => row.label).join("-"))}`,
    date,
    dayIndex,
...
function buildStraddleSchedule(): StraddleScheduledPuzzle[] {
  const state: StraddleScheduleBuildState = {
    rowLabelCounts: new Map(),
    signatures: new Set(),
    wordCounts: new Map(),
    wordLastSeen: new Map(),
  };
  const schedule: StraddleScheduledPuzzle[] = [];

  for (let dayIndex = 0; dayIndex < STRADDLE_SCHEDULE_DAYS; dayIndex += 1) {
    const date = addUtcDays(STRADDLE_SCHEDULE_START_DATE, dayIndex);
...
export function scoreStraddlePuzzleSatisfaction(
  puzzle: StraddleScheduledPuzzle,
): number {
  const words = puzzle.grid.flat();
  let score = 74;
...
export const STRADDLE_SCHEDULE: StraddleScheduledPuzzle[] =
  buildStraddleSchedule();

export function getStraddleScheduleEditorialAudit(
  schedule: StraddleScheduledPuzzle[] = STRADDLE_SCHEDULE,
): StraddleScheduleEditorialAudit {
  ...
  schedule.forEach((puzzle) => {
    puzzle.grid.flat().forEach((word) => {
      ...
      if (
        lastSeen !== undefined &&
        puzzle.dayIndex - lastSeen < STRADDLE_WORD_REUSE_SOFT_COOLDOWN_DAYS
      ) {
  ...
  const satisfactionScores = schedule.map(scoreStraddlePuzzleSatisfaction);
  ...
  return {
    ...
    wordReuse: {
      ...
      softCooldownDays: STRADDLE_WORD_REUSE_SOFT_COOLDOWN_DAYS,
    },
  };
}

export function getStraddlePuzzleForDate(
  dateKey: string,
): StraddleScheduledPuzzle | null {
  return STRADDLE_SCHEDULE.find((puzzle) => puzzle.date === dateKey) ?? null;
}

export function getStraddlePackPuzzleForDate(
  dateKey: string,
): StraddleScheduledPuzzle | null {
  if (dateKey < STRADDLE_SCHEDULE_START_DATE) return null;
  return getStraddlePuzzleForDate(dateKey);
}

export function getTodaysStraddlePuzzle(
  today = new Date(),
): StraddleScheduledPuzzle | null {
  return getStraddlePackPuzzleForDate(toDateKey(today));
}
