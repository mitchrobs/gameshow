...
import { resolveScreenAccent, useDaybreakTheme } from '../../constants/theme';
import { BUILD_ID } from '../../constants/build';
import { getTriviaLocalDateKey } from '../../data/trivia/date';
import { formatTriviaShareText } from '../../data/trivia/results';
import type {
  TriviaDifficulty,
  TriviaEpisode,
  TriviaFeed,
...
export function TriviaGameScreen({ runtime }: { runtime: TriviaFeedRuntime }) {
  ...
  const previewEpisode = useMemo(
    () => runtime.getTodayEpisode(selectedDifficulty),
    [runtime, selectedDifficulty]
  );
  const previewSummary = useMemo(
    () => runtime.getFeedSummary(selectedDifficulty),
    [runtime, selectedDifficulty]
  );
  ...
  const startRun = useCallback(() => {
    const nextEpisode = runtime.getTodayEpisode(selectedDifficulty);
    resetTimers();
